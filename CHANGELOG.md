...
- **`${env:NAME}` placeholders in service environment variables** — `environment.services.<name>.env` values may now reference engine-process environment variables via `${env:NAME}` syntax, resolved at topology-build time before container startup. An unset variable fails the suite before any container starts, naming the variable — never an empty-string substitution. The resolved value is visible via standard `docker inspect` output (inherent to container environment storage, not a security defect); authors should treat service-visible environment variables as non-confidential and use `${secret:…}` references for credentials requiring redaction.
...
- **Explicit service health-check configuration** — `environment.services.<name>.healthCheck:` allows health-check override or disabling. Two forms: `{ type: tcp, port: N }` (a TCP connect probe on the specified port, no HTTP request) or `{ type: http, path: "/..." }` (the explicit spelling of the default HTTP `/` health check). Omitting `healthCheck` on an `image:` HTTP service preserves today's default behaviour (HTTP probe on `/`). `type` values are case-sensitive.
- **Service-targeted Kafka publish/expect steps** — `mq-publish.kafka` and `mq-expect.kafka` now accept a `target` naming a declared service (previously dependency-only). At runtime, a service-targeted Kafka step currently fails closed as an environment error — the provider-side connection staging for services arrives with a later slice of this series.
- **Closed target resolution for HTTP and metrics steps** — `http.rest`, `http.soap`, and `metrics-assert.prometheus` now reject a `target` naming neither a declared service nor a declared dependency at validation time (`vouchfx validate`, before any container starts), naming the target and listing what is declared. Previously, an unknown target was accepted and failed only at runtime. Host-resource-contributed names (e.g. a `webhook-listen.http` listener) count as valid declared targets — but a host resource whose name collides with a declared service is now rejected outright, naming both surfaces. Such a suite previously validated and ran, with the listener silently shadowing the service, so a step targeting that name reached the engine's own listener rather than the system under test and could report a pass having never contacted it.
