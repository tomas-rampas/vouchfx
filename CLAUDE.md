...
- `environment` — `services` (system under test: `image:` preferred, or `project:` csproj, with optional `env` map for container environment variables using `${conn:<dependency>}` connection references resolved at topology-build time in the consumer's network context), `dependencies` (managed Aspire resources: postgres, kafka, …), optional `seed` (SQL/fixtures/warm-up applied after the topology is healthy), `imageRegistry` override (applies to `services` only, not `dependencies`), `imagePullPolicy` (parsed but not enforced at runtime).
