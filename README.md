...
> **Security note:** `validate` compiles your test in-process using the same Roslyn compiler as `run`, with no sandboxing. This is safe for suites you author and trust, but not for actively hostile input. The engine bounds `script.csharp` body size (64 KiB) and `.e2e.yaml` document size (1 MiB) as resource limits; these are sanity bounds, not a defence against the compiler crash. A determined hostile suite author can still craft code to crash or hang the in-process compiler (a stack overflow is uncatchable by design), and this cannot be fully prevented in-process. For use cases involving untrusted input (such as the vouchfx MCP server), isolate validation in a separate worker process — which is exactly what the MCP server does. Terminal diagnostic output is scrubbed of control characters and ANSI escape sequences at every known diagnostic output path, so a crafted error string cannot inject terminal escapes through those paths into a human-viewed report (`--json` output was already safe). Fuller in-process→out-of-process isolation of the engine's compile step is planned as a future enhancement.
...
- **`--events-stream <path>`** — writes the same JSON Lines event stream as `--events`, but incrementally to a tailable file as the run proceeds. The engine holds the write handle and grants shared read access; a tailing reader must open the file with shared read/write access (on Windows, `FileShare.ReadWrite`; on Unix, the file is readable immediately). UTF-8 without BOM. Useful for live tailing, CI progress streaming, and downstream consumers (such as the vouchfx MCP server). Step and step-attempt events are now emitted in real time as each step/attempt completes (not scenario-granular); the polling timeline of a RETRY step is observable live as attempts happen. In parallel mode, step lines from concurrently-running scenarios interleave by arrival order in the stream, but each event still carries `runId` and `stepId`, which consumers use to disambiguate aggregated streams. On a cancelled or faulted run, the live stream may contain partial per-step lines (up to and including the step that was executing) that the authoritative `--events` archive does not — the stream shows progress, the archive is the source of truth. Independent of `--events` — you can use both together. Best-effort: an unwritable path prints a diagnostic and does not affect the run's verdict.

All four flags accept `--parallel` and sequential runs; none works with `--watch` (which re-renders on each iteration rather than buffering one suite-wide stream). Parent directories are created as needed; existing files are overwritten.
...
- **[vouchfx-mcp](https://vouchfx-mcp.vouchfx.io/)** — Model Context Protocol (MCP) server for AI-assisted test authoring (not yet published as a tool). Provides schema validation, step-type catalogue lookup, suite execution, and event-stream diagnostics. ([source](https://github.com/tomas-rampas/vouchfx-mcp))
