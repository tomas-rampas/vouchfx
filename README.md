...
> **vouchfx is in public pre-release: `v1.0.0-alpha` builds are live on [NuGet.org](https://www.nuget.org/packages/vouchfx/) and [GitHub Releases](https://github.com/tomas-rampas/vouchfx/releases), and v1.0 GA is in preparation.** Install the CLI with `dotnet tool install --global vouchfx --prerelease`. The engine compiles `.e2e.yaml` declarative integration
...
`metrics-assert.prometheus`; `storage-assert.s3`; `trace-expect.otlp`; and `script.csharp`. All are governed across three tiers (Core / Verified / Community), all Apache-2.0.
