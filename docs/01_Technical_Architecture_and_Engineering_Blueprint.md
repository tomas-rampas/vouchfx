...
The runner's exit codes carry the verdict taxonomy all the way to the CI system, so that infrastructure breakage, inconclusive results, and genuine defects can be distinguished and handled separately. The mapping is controlled by optional flags, all off by default, which implement the principle that **only Fail breaks CI by default**.
