...
The runner's exit codes carry the verdict taxonomy all the way to the CI system, so that infrastructure breakage, inconclusive results, and genuine defects can be distinguished and handled separately. The mapping is controlled by optional flags, all off by default, which implement the principle that **only Fail breaks CI by default** — with one narrow, deliberate exception: a suite that declares a `security:` block the engine cannot confirm exits non-zero regardless of the flags, because an unconfirmable security assertion is not an infrastructure flake but an assertion the author explicitly wrote that the engine could not satisfy. Every other cause of an EnvironmentError is unaffected. The set of causes is open and is maintained in one place: see `docs/ci-integration.md` for the current list and the code each one carries.
...
| 3 | EnvironmentError | For `run`: the aggregate verdict was EnvironmentError (infrastructure breakage: unhealthy container, image-pull failure, seed failure, tunnel collapse). Off by default; breaks CI only when `--fail-on-env-error` is set. Exception: a declared `security:` block whose post-health-gate confirmation probe fails exits 3 unconditionally, independent of the flag. For `list`, `schema`, `validate`, `scaffold`, `plan`: incomplete catalogue metadata (a registered provider lacks a schema fragment). | `--fail-on-env-error` (run only) |
| 4 | Inconclusive | The aggregate verdict was Inconclusive (timeout, network partition outlasted grace, upstream capture unmet — the engine could not determine correctness). Off by default; breaks CI only when `--fail-on-inconclusive` is set. Two exceptions exit 4 unconditionally, independent of the flag: a `run` where every discovered scenario fails to parse; and a declared `security:` block the engine refuses before any container starts, whatever refused it — the security preflight (a certificate or artefact path escaping the suite directory or absent, an artefact `target` that is not an absolute in-container file path, a `profile` with no wiring for the target's kind), the schema (any schema error at all in a document that declares `security:`, wherever in the document it sits — an error inside the block itself, and the per-kind narrowing of `profile`, are cases of that rule rather than its definition), the refusal of a `clientKeyPassword` that is not one whole `${secret:…}` reference naming a known source, or the multi-scenario directory-layout guard. `docs/ci-integration.md` holds the maintained list. | `--fail-on-inconclusive` |
...
- **`--fail-on-inconclusive`** — When set, an Inconclusive verdict exits with code 4 instead of 0. Use this to gate on results the engine could not decide (timeout, partition, unmet upstream captures), in scenarios where deterministic decisions are required. Note two cases that already exit 4 regardless of this flag: a `run` where every discovered scenario fails to parse, and a declared `security:` block the engine refuses before any container starts — whatever refused it, since a declaration nothing downstream ever validates is one the engine cannot confirm. `docs/ci-integration.md` enumerates the refusals currently in the engine and is the surface kept current as they change.
