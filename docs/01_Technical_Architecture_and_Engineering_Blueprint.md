...
The runner's exit codes carry the verdict taxonomy all the way to the CI system, so that infrastructure breakage, inconclusive results, and genuine defects can be distinguished and handled separately. The mapping is controlled by optional flags, all off by default, which implement the principle that **only Fail breaks CI by default** — with one narrow, deliberate exception: a suite that declares a `security:` block the engine cannot confirm exits non-zero regardless of the flags, because an unconfirmable security assertion is not an infrastructure flake but an assertion the author explicitly wrote that the engine could not satisfy. Every other cause of an EnvironmentError is unaffected. The causes are not a list to be maintained: the engine derives the exception from one predicate rather than from a set of named doors, so a cause either satisfies it or does not, without being enumerated anywhere. A declared `security:` block counts as unconfirmed when a refusal that started no container left some declared target unconfirmed, when the schema rejected the declaration itself, or when the post-health-gate confirmation probe measured the declaration not to hold. A topology that came **up** and then failed its health gate is deliberately outside the predicate — a run whose *only* fault is that gate exits 0 (issue #390 tracks narrowing it). The gate failure never clears a refusal the same run already recorded, so a secured suite carrying an authoring refusal beside it still exits non-zero on that refusal. See `docs/ci-integration.md` for the CI-facing treatment and the code each outcome carries.
...
| 4 | Inconclusive | The aggregate verdict was Inconclusive (timeout, network partition outlasted grace, upstream capture unmet — the engine could not determine correctness). Off by default; breaks CI only when `--fail-on-inconclusive` is set. Two exceptions ignore the flag: a `run` where every discovered scenario fails to parse; and a declared `security:` block that a refusal starting no container left unconfirmed, including a schema error located in the declaration itself. Only a refused **scenario** the run went on to confirm past escapes that; a document refused before it became a scenario does not (see the `--fail-on-inconclusive` note below). The second is a property rather than a set of named doors — a security preflight, a schema error anywhere in the document, an unresolvable `script.csharp` `file:`, a bad `${conn:…}` or `${secret:…}` reference, a protocol conflict and the multi-scenario directory-layout guard are all instances of it, none of them a definition of it. See `docs/ci-integration.md` for the CI-facing treatment. | `--fail-on-inconclusive` |
