...
| 3 | EnvironmentError | For `run`: the aggregate verdict was EnvironmentError (infrastructure breakage: unhealthy container, image-pull failure, seed failure, tunnel collapse). Off by default; breaks CI only when `--fail-on-env-error` is set. For `list`, `schema`, `validate`, `scaffold`, `plan`: incomplete catalogue metadata (a registered provider lacks a schema fragment). | `--fail-on-env-error` (run only) |
