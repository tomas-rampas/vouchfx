...
For developers iterating on a single `.e2e.yaml` file, the CLI runner's `--watch` flag compresses the author→run loop by eliminating repeated topology rebuilds. When invoked with `vouchfx run <file> --watch`, the runner: builds the topology once, runs the scenario, then watches the file for changes. **On each save, every pre-topology gate that applies to a single file runs first** — schema validation, the provider-pipeline compile (which carries the security-artefact preflight, the security-profile wiring check and the one-target-one-staged-form protocol-conflict guard) and the secret-reference walk, the last two together as one door reporting both faults. A save any of them refuses is reported and the loop keeps watching, with no container started and the kept topology, if any, untouched. The guards a shared-topology `run` additionally applies — shared-`environment` divergence, the all-early stop, the seed-root/environment single-source rule, security base-directory divergence, and the suite-level protocol-conflict union — correctly do not run here: each exists because one topology serves N scenarios, and watch mode's unit is one file, for which the per-scenario checks above are complete. Only a save that passes them all reaches the reuse-vs-rebuild decision. That decision is taken on a **topology fingerprint**: a digest over every input the topology would be built from — the serialised `environment` block, the seed base directory, the app-host assembly name, the startup timeout, and the two protocol target sets derived from the `steps`. If the fingerprint is unchanged (the common case during iteration — a step's body, headers, assertions or URL path changed, but no *targeted resource name* did), the scenario re-runs against the existing topology; if it changed, the old topology is disposed, a new one is built, and the scenario runs against it. The target sets are part of the digest because they change what the topology *is*, not merely what is claimed about it: one decides whether an endpoint-less `project:`-form service is refused or is an untargeted worker, the other decides whether a service is staged as a bare `host:port` bootstrap authority or as a URL. The cost of that breadth is bounded and converges — at worst one extra rebuild per newly targeted service in a session. The reuse path first resets the database via Respawn, then re-applies the seed block to restore the freshly-provisioned baseline — matching the baseline of a plain `vouchfx run` so each watch run starts from identical state. Compile errors and run failures are reported concisely and the watch loop keeps running, so the developer can edit again without re-running from the shell. The `--watch` and `--parallel` flags are mutually exclusive (combining them is a usage error), as watch mode targets the single-file edit-run loop whilst parallelism fans multiple scenarios across independent topologies.
...
| Customer's application secrets | YAML to runtime | Secret value leaks via report, log, captured-variable thread. | ${secret:…} references resolved at run time and never at compile time (a step's field at step execution; `security.clientKeyPassword` at first use of the certificate material — §17.1.1), so no value is ever baked into the emitted IL or the reproducibility envelope; typed SecretString return that the renderer is contractually forbidden to log; redaction at source, not at sink (see 11.5). |
...
**The frozen surface includes CSX helper bodies, not only signatures.** `Vouchfx.Sdk` ships CSX helper classes — `SecretHelper`, `SubstituteHelper`, `SecurityHelper`, `KafkaSecurityHelper` — whose entire public surface is a single `public const string Source` holding the text of a static class that providers splice into `CsxFragment.RequiredHelpers`. Two properties make that text binding rather than incidental. A `const` is inlined into every consuming assembly at *that assembly's* compile time, so a provider built against an earlier SDK carries the earlier body verbatim and never re-reads the shipped `Vouchfx.Sdk` for it; and `CsxAssembler` deduplicates helpers by exact source text, refusing with a `CsxAssemblyException` any suite in which one helper class arrives with two different bodies (§13.3.1). Editing a helper body therefore breaks an out-of-tree provider still built against the previous SDK, at assembly time, in any suite that mixes it with an in-tree provider **splicing the same helper** — for `SubstituteHelper` (spliced by 22 of the 25 Core providers) and `SecretHelper` (18) that is in practice any suite, while `SecurityHelper` (3) and `KafkaSecurityHelper` (2, the Kafka pair) collide more narrowly — a breaking change to the v1 contract by any measure. The reflected signature cannot see it: the golden records `field const System.String Source`, which is byte-identical whichever text the constant holds. A companion golden, `vouchfx-sdk-helper-sources.v1.txt`, closes that gap by pinning a SHA-256 of each helper's `Source` value; helpers are discovered by shape (any public type exposing a `public const string Source`) rather than by name, so one added later is pinned the moment it compiles. Both goldens regenerate under the same `VOUCHFX_REGEN_SDK_CONTRACT` flag, because they pin two halves of one contract. The v2 migration that eliminates this hazard rather than merely policing it is to declare each helper's `Source` as `static readonly` instead of `const`: every provider would then read the single loaded `Vouchfx.Sdk` at run time, no body could be inlined into a consumer at its own compile time, and `CsxAssembler`'s same-name-different-text refusal would become unreachable **for the SDK-supplied helpers** by construction (a provider-authored helper class that embeds step-specific data still trips it, which is that check's primary purpose) — a change unavailable inside v1.x only because it mutates an existing member of the golden-frozen surface.
...
The event-record surface — the top-level event records and all nested value records reachable from them on the wire — is **frozen at v1 for the v1.x engine series**, enforced by a golden-file CI gate (`EventContractFreezeTests`) that asserts every property name, CLR type, and `[JsonPropertyName]` wire key is byte-for-byte identical to the committed golden. Any change to the JSON-Lines wire contract breaks every consumer (terminal renderer, HTML report, JUnit XML, dashboard, Healer agent), so this freeze is non-negotiable.

The frozen set is stated here as a property rather than as a roster, because a roster copied into prose goes stale the moment a record is added and then misleads a consumer into thinking a real wire record is unfrozen and unsafe to parse. The property: **every public record declared in the `Vouchfx.Engine.Abstractions.Events` namespace is frozen**, together with the nested value records that ride on the wire from a different namespace (`SecretReferenceDigest` and `FixtureDigest`, both nested in the reproducibility envelope and both declared under `Vouchfx.Engine.Abstractions.Reproducibility`). The gate's own `s_eventRecords` array is the authoritative enumeration, and it is more than a list a reader has to trust: a companion completeness test reflects over that namespace and fails — naming the uncovered type — until a newly added public record is either listed in the frozen set or written into an explicitly commented exclusion set, which is empty and is meant to stay empty. A record therefore cannot enter the wire surface unfrozen, and the array cannot silently fall behind the namespace it guards.
...
### 14.4.3 Transport-notice events

The stream carries a `transport-notice` event type for the two advisories the engine raises about the endpoint a targeted service is addressed on: that it selected a plaintext listener for a `project:`-form service while an https listener was also available, and that the run addresses an https listener for which it configures no client trust material of its own. Both advisories are printed to the terminal by their own route, and the event exists so that a machine consumer sees them too. That matters most for the second, because the handshake failure it warns about surfaces as an **Environment error** and so exits 0 by default (§12.1): without the event, a machine consumer of such a run sees green and nothing about why.

**The record's reach is `--events` and `--events-stream`, and that scope is the whole of what the event adds.** The JUnit and HTML renderers take their `default:` arm on an event type they do not recognise — the forward-compatibility guarantee every §14 renderer carries — so a pipeline whose only archived artefacts are `results.xml` and `report.html` still gets a green report with nothing in it about the transport. Rendering the record in those two is a separate change and is not implemented.

One record type discriminated by a `kind` token, rather than two record types. The two advisories are siblings, and a single record is what keeps their treatment in the artefacts from diverging the way it can when each has a producer of its own. The vocabulary is closed — `plaintext-downgrade` and `no-engine-trust` — but not closed at two: a third advisory is absorbed by adding a token, which is additive, and a consumer meeting a token it does not recognise surfaces it as an unrecognised transport advisory rather than discarding the record. `no-engine-trust` is worded to say what is absent (engine-configured trust) rather than to claim the transport went unverified, because with no `security` block the platform's own trust store still validates the chain; on this wire the token is the whole of what a consumer receives, so it must not overstate.

Beyond the envelope fields every record carries, the payload is `kind` (the discriminator, carrying a token from the closed vocabulary above — a required payload field, not an envelope one, and the first payload field to branch on, though `type` comes first of all: `kind` is the second wire property of that name, because `step-started` already carries a `kind` holding the step type, e.g. `http.rest`), `service` (the name declared under `environment.services`), `selectedEndpoint` (the endpoint steps addressing that service will use), an optional `rejectedEndpoint` (present only on the downgrade kind — nothing is rejected in the trust case), and an optional `replayed`, set only where watch mode re-reports an advisory raised by an earlier topology build under its own staleness qualifier. That last field needs its qualifier stated with it: `--watch` is wired to no report artefact whatever — `--events`, `--events-stream`, `--junit` and `--html` alike — so `replayed` is a property of the record rather than of anything a watch run writes, and no consumer reads one today. Both endpoint fields carry endpoint **names**, never a resolved URL or a `host:port` authority, and that restriction is what makes the record safe to archive as a CI artefact. The fields are neither secret-scrubbed nor display-sanitised, and both omissions are deliberate: every member is a closed engine token or a declared name into which no secret reference is substitutable, `System.Text.Json` escapes control characters on the wire in any case, and rewriting the service name would break the only correlation a consumer has back to the suite it came from.

Two further properties follow from what an advisory is about. It is **run-level, not scenario-level**: the advisory belongs to the topology a run built, and one topology can serve many scenarios, so the record carries no `scenarioId` — and where a single topology serves a whole suite, its envelope `runId` deliberately resolves to no scenario at all, because attributing a topology-wide fact to one named scenario would make a renderer state something false about the others. `service` is the correlation key a consumer wants. And the record carries **no verdict**: emitting it moves neither the taxonomy of §12.1 nor the exit code, it is emitted whether or not any renderer is attached, and its cardinality is the terminal's — at every site, the number of records emitted equals the number of advisories printed. Whether a given renderer surfaces the record is that renderer's own decision; the stream is the contract, and a renderer that does not recognise the type ignores it under the forward-compatibility guarantee above.
...
A throw from `CanRender` or `RenderDiff` is contained at the render seam: the step's diff is omitted, one diagnostic line naming the step kind, provider type, member, and exception type is written to the run's diagnostics writer, and the verdict, exit code, and every report artefact are unaffected (issue #485).
...
|---|---|---|
| **Declarative fixtures (default)** | An environment-level seed block declares reference SQL files, applied against a relational dependency (postgres, sqlserver, or mysql) before the suite starts. The engine applies them after the topology is healthy and before the first step runs. Document-store fixtures and broker warm-up messages were wired-but-deferred stubs in an earlier draft (never used anywhere in the repository — they read and content-hashed a fixture but never wrote or published it) and were removed from the v1 language before general availability; `sql` is the only seed kind today. | Reference and lookup data; a known baseline shared across a suite; anything that is the same for every run. |
...
## 16.3.1 Coverage analysis and the Planner

Complementary to the runner is the **Planner** (`vouchfx plan`), a deterministic, read-only analysis tool that answers "what should I test next?" by intersecting three engine-side sources: the declared suite set (the `.e2e.yaml` files on disk), the exercised reality (the JSON Lines run history from `--events`), and the available vocabulary (the registered step catalogue). The Planner emits a coverage-and-gap report with ten finding kinds: five coverage gaps (suite never run, step never exercised, dependency not asserted, vocabulary missing, service missing HTTP step), four history-health signals (step stale, flaky, fragile, or inconclusive-prone), and one identity finding (suite identity ambiguous — a historical run's suite identity could not be resolved to exactly one currently-declared suite). Only the five coverage gaps count towards `--fail-on-gap`; history-health and identity findings never do. Every gap finding carries structured hints the `scaffold` subcommand needs, so a user can pick a gap, generate a skeleton test, and validate it without re-deriving anything. The Planner never writes a suite file, never calls a model, and never claims coverage it cannot evidence from its declared inputs; exit codes are 0 (successful analysis, regardless of gaps), 2 (usage error), 3 (incomplete catalogue metadata), or 5 (gaps found, only when `--fail-on-gap` is set). See [Planner documentation](planner.md) for the full reference.

## 16.4 Exit codes

The runner's exit codes carry the verdict taxonomy all the way to the CI system, so that infrastructure breakage, inconclusive results, and genuine defects can be distinguished and handled separately. The mapping is controlled by optional flags, all off by default, which implement the principle that **only Fail breaks CI by default** — with four narrow, deliberate exceptions: (1) a suite that declares a `security:` block the engine cannot confirm exits non-zero regardless of the flags, because an unconfirmable security assertion is not an infrastructure flake but an assertion the author explicitly wrote that the engine could not satisfy; (2) any parse failure never exits 0 regardless of the flags; (3) a suite that parsed and was then refused before any scenario executed, which leaves the run Inconclusive, never exits 0 regardless of the flags either; and (4) a scenario refused at one of the engine's **provider- or engine-surface guards** — a provider's `Bind`, `Validate`, `Resources`, `HostResources`, `Emit` or `CompileReferenceAssemblies` threw, or the assembler refused the CSX fragments a provider emitted — never exits 0 either, whatever its siblings did. Exceptions (2) and (3) are siblings — both are no-verdict rules, differing only in what closed off a verdict (an unreadable document versus a suite refused before anything ran) — counted separately here rather than as one grouped exception. The second, third and fourth exceptions are stated as "never exits 0" rather than "exits 4" deliberately: every one of these guards is conditioned on the code so far being Success, so none overrides a code another rule already chose — a parse failure beside a failing scenario still exits 1, and beside a gated environment error, 3. The fourth exists because exception (3) is conditioned on nothing having executed, and so stopped protecting a defective scenario the moment a sibling ran: the same provider defect exited 4 alone in a directory and 0 beside one passing scenario, deciding on the sibling rather than on the defect. It is keyed on where the failure was raised, never on the verdict, precisely so that a genuine execution-time Inconclusive and a genuine infrastructure fault keep the opt-in gates §12.1 gives them. Every other cause of an EnvironmentError is unaffected by these rules. The causes are not lists to be maintained: the engine derives the exceptions from predicates rather than from sets of named doors, so a cause either satisfies one or does not, without being enumerated anywhere. A declared `security:` block counts as unconfirmed when a refusal that started no container left some declared target unconfirmed, when the schema rejected the declaration itself, or when the post-health-gate confirmation probe measured the declaration not to hold. A topology that came **up** and then failed its health gate is deliberately outside the security predicate — a run whose *only* fault is that gate exits 0 (issue #390 tracks narrowing it). The gate failure never clears a refusal the same run already recorded, so a secured suite carrying an authoring refusal beside it still exits non-zero on that refusal. **None of this applies to `--watch`, which is not a CI mode.** `WatchRunner` returns only `UsageError` for a selection that is not exactly one parseable file, and `Success` otherwise; it never calls the verdict-to-exit mapping at all, so no verdict and no exception on this page can make a watch session exit non-zero. Every exit code and every exception described here is about `run` and `run --parallel`. See `docs/ci-integration.md` for the CI-facing treatment and the code each outcome carries.
...
| 2 | UsageError | Bad arguments, missing path, invalid `--parallel` value, `--watch` combined with `--parallel`, or unrecognised option/flag. The suite never ran. Also used by `plan`, `validate`, `schema`, `list` and `scaffold` for usage errors (bad path, missing parent directory for `--output`). Additionally used by `--changed-since` selection failures: an invalid git reference, git not installed or the directory not being a git repository, or a git invocation exceeding its per-call time budget or whose output capture fails. On Windows, "git not installed" also covers a `.cmd`/`.bat` shim: resolution matches an `.exe` candidate only, deliberately, because a shim launches through `cmd.exe` and reopens argument injection on a flag that splices an author-supplied ref into its own argv. | – |
| 3 | EnvironmentError | For `run`: the aggregate verdict was EnvironmentError (infrastructure breakage: unhealthy container, image-pull failure, seed failure, tunnel collapse). Off by default; breaks CI only when `--fail-on-env-error` is set. Exception: a declared `security:` block whose post-health-gate confirmation probe fails exits 3 unconditionally — independent of `--fail-on-env-error` specifically (it is decided inside `ExitCodes.FromVerdict` itself, alongside that flag, not by overriding a code some other rule already chose); so does a secured suite refused by the shared-`environment` divergence guard, which stamps the suite-level EnvironmentError onto every scenario and writes the run's requested reports. The same divergence in an unsecured suite still exits 0 — and, since that guard began completing rather than returning bare, now writes those reports too, where previously it wrote none. For `list`, `schema`, `validate`, `scaffold`, `plan`: incomplete catalogue metadata (a registered provider lacks a schema fragment). | `--fail-on-env-error` (run only) |
| 4 | Inconclusive | The aggregate verdict was Inconclusive (timeout, network partition outlasted grace, upstream capture unmet — the engine could not determine correctness). Off by default; breaks CI only when `--fail-on-inconclusive` is set. Four exceptions ignore the flag: (1) a `run` containing any parse failure, whether or not a sibling parsed; (2) a `run` in which nothing executed — the suite parsed fine and was then refused before any scenario ran (schema rejection, unresolvable secret reference, malformed dependency `env:`, protocol conflict, etc.); (3) a declared `security:` block that a refusal starting no container left unconfirmed; and (4) a `run` in which any scenario was refused at a provider- or engine-surface guard, whatever its siblings did. What distinguishes (4) is not that it survives an executing sibling — (1) and (3) do too — but that it keys on WHERE the failure was raised rather than on what the run managed to do or declare. Note that (2) and (4) are not nested: (2) covers every pre-topology refusal but only while nothing ran, and (4) covers only the provider/engine-surface subset but survives a passing sibling, which is why a defective scenario alone in a directory is already covered by (2) and needed (4) only once it acquired a sibling. An ordinary authoring refusal beside a scenario that ran falls into neither and still exits 0 (issue #514 tracks narrowing it), and an exception escaping to `--parallel`'s per-slot catch-all is outside (4) because that frame cannot tell an engine defect from a container falling over (issue #486). The first two of those collectively: any parse failure, or a suite refused before any scenario executed whose aggregate verdict is Inconclusive, never exits 0. A run that executed nothing and carries an `EnvironmentError` instead — a topology that failed to start, an UNSECURED suite whose scenarios declared divergent `environment` blocks — keeps that verdict's own gate and still exits 0 by default (issue #390 tracks narrowing it) — provided that is the run's only fault, since a Pass-B provider defect followed by a topology that fails to start carries both and exits 4 BY DEFAULT through exception (4) — these guards are conditioned on the code so far rather than on the aggregate verdict, so with `--fail-on-env-error` set that run exits 3 instead, the gate having already taken the code off Success before exception (4) is consulted. A SECURED suite refused by that divergence guard exits 3 through exception (3) instead, whatever the flags. The two refusal cases are a property rather than a set of named doors — a security preflight, a schema error anywhere in the document, an unresolvable `script.csharp` `file:`, a bad `${conn:…}` or `${secret:…}` reference, a protocol conflict and the multi-scenario directory-layout guard are all instances of them, none of them a definition of them. Only a refused **scenario** the run went on to confirm past escapes that; a document refused before it became a scenario does not. See `docs/ci-integration.md` for the CI-facing treatment. | `--fail-on-inconclusive` |
| 5 | Gaps found | The Planner's `plan` subcommand found at least one coverage or vocabulary gap (`suite-never-run`, `step-never-exercised`, `dependency-not-asserted`, `dependency-missing-step-type`, or `service-missing-http-step` finding) AND the caller opted in via `--fail-on-gap`. A successful analysis exits 0 regardless of gap count unless `--fail-on-gap` is set — gaps are data. | `--fail-on-gap` (plan only) |

**`--changed-since` selection failures are a usage error, not an infrastructure fault, and this is a decision rather than an accident (issue #521).** Every way a `--changed-since` change-set can fail to be computed — an invalid git reference, git not installed or not resolvable on `PATH`, the directory not being a git repository, a git invocation exceeding its per-call time budget, or a failed output capture — is caught as `ChangeSetException` and mapped to exit 2 (UsageError), unconditionally: exit 2 does not pass through the verdict-to-exit mapping, so none of the four unconditional-non-zero exceptions above, and neither opt-in flag, can move it. The reasoning: the operator supplied a `--changed-since` argument the environment cannot satisfy, and a selection that fails before any scenario runs — before a topology is even attempted — is the same class of problem as a bad path or an invalid `--parallel` value, which this table already treats as exit 2. This holds even when the underlying cause is not, in the ordinary sense, something the operator typed wrong — git missing from a CI image, or a git invocation that outlasted its budget, are facts about the environment, not typos — but the selection language is CLI input like any other on this page, and failing to resolve it is classified the same way any other unsatisfiable CLI input is. The cost of that choice is stated plainly rather than hidden: a git-less image reads identically, at the exit-code level, to a mistyped `--changed-since` ref; only the printed message, never the code, tells the two apart. A graceful `--shutdown-on-stdin-eof` stop **during** selection is not an exception to this rule so much as a different rule entirely: it is a requested shutdown, not a usage error, so it is routed through the same classification an EOF stop during execution uses and exits 4 (`Inconclusive`, issue #502) — never 2. That is the boundary between the two codes a `--changed-since` selection can produce: whether the operator asked for a selection the environment cannot satisfy (2), or asked the run to stop while selection was still in flight (4).
...
- **`--fail-on-inconclusive`** — When set, an Inconclusive verdict exits with code 4 instead of 0. Use this to gate on results the engine could not decide (timeout, partition, unmet upstream captures), in scenarios where deterministic decisions are required. Note four cases this flag does not govern — all break CI whether or not it is set: (1) a `run` containing any parse failure, (2) a `run` where the suite parsed fine but was then refused before any scenario executed, (3) a declared `security:` block that a refusal starting no container left unconfirmed, since declarations nothing downstream of that refusal ever validates are ones the engine cannot confirm, and (4) a `run` in which any scenario was refused at a provider- or engine-surface guard, whatever its siblings did. The first two of those collectively: any parse failure, or a suite refused before any scenario executed whose aggregate verdict is Inconclusive, never exits 0. A run that executed nothing and carries an `EnvironmentError` instead — a topology that failed to start, an UNSECURED suite whose scenarios declared divergent `environment` blocks — keeps that verdict's own gate and still exits 0 by default (issue #390 tracks narrowing it) — provided that is the run's only fault, since a Pass-B provider defect followed by a topology that fails to start carries both and exits 4 BY DEFAULT through exception (4) — these guards are conditioned on the code so far rather than on the aggregate verdict, so with `--fail-on-env-error` set that run exits 3 instead, the gate having already taken the code off Success before exception (4) is consulted. A SECURED suite refused by that divergence guard exits 3 through exception (3) instead, whatever the flags. The second and third cases both involve a refusal, and carry weight: the word carrying that weight is **scenario**: a suite whose refused *scenario* sits beside siblings whose confirmation probe did confirm every declared target exits 0, because a suite's scenarios must declare a byte-identical `environment` block — so the declaration that probe confirmed is the refused scenario's own, and the refusal is no longer the last word on it. **A document refused before it ever became a scenario is outside that carve-out**: one that parsed and was then refused for its contents (an unknown step type, a duplicate step id) is confirmed by nothing, since nothing established that its `environment` block is the one the topology started from, and it exits non-zero whatever its siblings confirmed. Do not read "document" and "scenario" as interchangeable in this section. There is no list of refusals to consult or to keep current; see `docs/ci-integration.md` for the CI-facing treatment.
...
Secrets are never written literally in a test file. A field that needs one carries a reference of the form `${secret:source/path}`, which the engine resolves at run time from a configured secret source — never at compile time. Exactly *when* at run time is a property of the field that carries the reference, not of the reference: a step's own field resolves immediately before that step executes, while `security.clientKeyPassword`, declared on a service or a `kafka` dependency (§3.2.6b of the DSL specification), resolves at first use of the certificate material it unlocks, which is after the topology is up and before any step runs. The resolved value lives only in memory for the duration of the run and is never written into a report, a log, or a captured-variable thread — the reporting layer redacts any value that originated from a secret reference, showing `${secret:…}` in its place. The reference is what appears in source control; the value never is.
...
### 17.1.1 Resolution happens at run time, never at compile time

An important architectural point: secrets are **not** interpolated into the generated CSX at compile time. If they were, the resolved values would be baked into the compiled assembly's IL — which would defeat the compile-once optimisation of Section 5 (every run with different secrets would produce different IL), corrupt the reproducibility envelope (its content hash would depend on values that should never appear in it), and force a recompilation on every secret rotation. Instead, the compiler emits a call to the global context's `Secrets` accessor, and resolution happens at run time. The reproducibility envelope therefore hashes the secret **references** — the YAML strings — not the resolved values, which is exactly the correct behaviour.

**"Never at compile time" is the invariant this section fixes; "at step execution" is not.** The moment of resolution belongs to the field that carries the reference. A step's own substitutable field resolves at the moment that step executes, which is the case the emitted code below illustrates. An environment-level field need not: `security.clientKeyPassword` resolves at first use of the certificate material it unlocks — after the topology is healthy and before any step has run, which in practice means the pre-run confirmation probe. Both are run time, both leave the compiled script free of any value, and both leave the envelope hashing the reference. Any rule stated over secrets generally must therefore be stated as *never at compile time*; a rule stated as *at step execution* is a statement about one kind of field and is false of the other.
...
A `ResolvedSecretLedger` records every value successfully revealed by `Resolve`, and the runner applies this ledger as a defence-in-depth scrub at the reporting boundary: before a step's observation text enters the JSON Lines event stream, every occurrence of a recorded secret value is replaced with `***REDACTED***` in both its raw form and its JSON-escaped form. This catches the realistic accident — a secret appearing unredacted in an exception message — without chasing theoretical transforms that are explicitly out of scope.
...
**How far a ledger reaches is decided by whoever constructs it, and the engine shares one across a whole run.** The topology's own pre-run confirmation probe and each scenario's accessor record into the same instance, which is what makes a value resolved on one of those paths — `security.clientKeyPassword`, resolved by the probe before any step runs — scrubbable from text emitted on another. Under `--watch` the shared instance is wider still: one ledger for the whole watch session, because the topology is kept across saves and the sink that receives a rebuild's failure captures the ledger before the rebuild resolves anything, so a narrower scope would scrub against the wrong object.

That breadth has a cost, and it is stated rather than argued away: the ledger retains a plaintext `System.String` copy of each revealed value for its own lifetime — you cannot scrub a value you do not hold — so the retained-plaintext window is the whole run, and under `--watch` the whole session, which is a developer's afternoon rather than a run's few minutes. Nothing clears a recorded value within a session; restarting `--watch` starts an empty ledger. A wider window also widens the scrub: every recorded value goes on being replaced wherever it appears, so a short or common one over-redacts later output. That is the deliberate direction to fail in — over-redaction is conspicuous, while a length floor would silently under-redact a short secret.

The memory model is clean regardless of scope: the ledger holds plain `System.String` values in the Default ALC (owned by the `SecretAccessor`s that record into it), so recording from inside the collectible script's `accessor.Resolve(…)` call runs a Default-ALC method body that adds to a Default-ALC set. No static handles bridge the collectible-ALC boundary, and the ledger is collected with its last holder.
...
| Report rendering quality varies across providers because diff rendering is provider-supplied. | Provider-supplied renderers are reviewed at Vouched badge award against a corpus of representative failures; a provider omitting `IStepDiffRenderer` will have its diff line omitted from the report but the verdict and all other output are unaffected (issue #485). | An unbadged Community provider with a poor or missing renderer produces less useful failure reports; this is documented in the tier guarantees. |
