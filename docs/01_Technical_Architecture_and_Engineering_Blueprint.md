...
The runner's exit codes carry the verdict taxonomy all the way to the CI system, so that infrastructure breakage, inconclusive results, and genuine defects can be distinguished and handled separately. The mapping is controlled by optional flags, all off by default, which implement the principle that **only Fail breaks CI by default** — with two narrow, deliberate exceptions: (1) a suite that declares a `security:` block the engine cannot confirm exits non-zero regardless of the flags, because an unconfirmable security assertion is not an infrastructure flake but an assertion the author explicitly wrote that the engine could not satisfy; and (2) any parse failure, or a suite that parsed and was then refused before any scenario executed, never exits 0 regardless of the flags. That second exception is stated as "never exits 0" rather than "exits 4" deliberately: both guards are conditioned on the code so far being Success, so neither overrides a code another rule already chose — a parse failure beside a failing scenario still exits 1, and beside a gated environment error, 3. Every other cause of an EnvironmentError is unaffected by these rules. The causes are not lists to be maintained: the engine derives the exceptions from predicates rather than from sets of named doors, so a cause either satisfies one or does not, without being enumerated anywhere. A declared `security:` block counts as unconfirmed when a refusal that started no container left some declared target unconfirmed, when the schema rejected the declaration itself, or when the post-health-gate confirmation probe measured the declaration not to hold. A topology that came **up** and then failed its health gate is deliberately outside the security predicate — a run whose *only* fault is that gate exits 0 (issue #390 tracks narrowing it). The gate failure never clears a refusal the same run already recorded, so a secured suite carrying an authoring refusal beside it still exits non-zero on that refusal. See `docs/ci-integration.md` for the CI-facing treatment and the code each outcome carries.
...
| 4 | Inconclusive | The aggregate verdict was Inconclusive (timeout, network partition outlasted grace, upstream capture unmet — the engine could not determine correctness). Off by default; breaks CI only when `--fail-on-inconclusive` is set. Three exceptions ignore the flag: (1) a `run` containing any parse failure, whether or not a sibling parsed; (2) a `run` in which nothing executed — the suite parsed fine and was then refused before any scenario ran (schema rejection, unresolvable secret reference, malformed dependency `env:`, protocol conflict, etc.); and (3) a declared `security:` block that a refusal starting no container left unconfirmed. The first two of those collectively: any parse failure, or a suite refused before any scenario executed whose aggregate verdict is Inconclusive, never exits 0. A run that executed nothing and carries an `EnvironmentError` instead — a topology that failed to start, an UNSECURED suite whose scenarios declared divergent `environment` blocks — keeps that verdict's own gate and still exits 0 by default (issue #390 tracks narrowing it). A SECURED suite refused by that divergence guard exits 3 through exception (3) instead, whatever the flags. The two refusal cases are a property rather than a set of named doors — a security preflight, a schema error anywhere in the document, an unresolvable `script.csharp` `file:`, a bad `${conn:…}` or `${secret:…}` reference, a protocol conflict and the multi-scenario directory-layout guard are all instances of them, none of them a definition of them. Only a refused **scenario** the run went on to confirm past escapes that; a document refused before it became a scenario does not. See `docs/ci-integration.md` for the CI-facing treatment. | `--fail-on-inconclusive` |
...
- **`--fail-on-inconclusive`** — When set, an Inconclusive verdict exits with code 4 instead of 0. Use this to gate on results the engine could not decide (timeout, partition, unmet upstream captures), in scenarios where deterministic decisions are required. Note three cases this flag does not govern — all break CI whether or not it is set: (1) a `run` containing any parse failure, (2) a `run` where the suite parsed fine but was then refused before any scenario executed, and (3) a declared `security:` block that a refusal starting no container left unconfirmed, since declarations nothing downstream of that refusal ever validates are ones the engine cannot confirm. The first two of those collectively: any parse failure, or a suite refused before any scenario executed whose aggregate verdict is Inconclusive, never exits 0. A run that executed nothing and carries an `EnvironmentError` instead — a topology that failed to start, an UNSECURED suite whose scenarios declared divergent `environment` blocks — keeps that verdict's own gate and still exits 0 by default (issue #390 tracks narrowing it). A SECURED suite refused by that divergence guard exits 3 through exception (3) instead, whatever the flags. The second and third cases both involve a refusal, and carry weight: the word carrying that weight is **scenario**: a suite whose refused *scenario* sits beside siblings whose confirmation probe did confirm every declared target exits 0, because a suite's scenarios must declare a byte-identical `environment` block — so the declaration that probe confirmed is the refused scenario's own, and the refusal is no longer the last word on it. **A document refused before it ever became a scenario is outside that carve-out**: one that parsed and was then refused for its contents (an unknown step type, a duplicate step id) is confirmed by nothing, since nothing established that its `environment` block is the one the topology started from, and it exits non-zero whatever its siblings confirmed. Do not read "document" and "scenario" as interchangeable in this section. There is no list of refusals to consult or to keep current; see `docs/ci-integration.md` for the CI-facing treatment.
