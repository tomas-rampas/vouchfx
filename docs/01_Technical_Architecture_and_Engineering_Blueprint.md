...
| 2 | UsageError | Bad arguments, missing path, invalid `--parallel` value, `--watch` combined with `--parallel`, or unrecognised option/flag. The suite never ran. Also used by `plan`, `validate`, `schema`, `list` and `scaffold` for usage errors (bad path, missing parent directory for `--output`). | – |
