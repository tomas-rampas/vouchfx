...
The third limit applies to a broker declared as a **service**, and is worth designing around before you write the suite. The engine stages that service's endpoint as a bootstrap authority a Kafka client can use directly, and its own pre-run confirmation reaches the broker over exactly that address. A *step*, however, does more than connect to the bootstrap: its client asks the broker where the partition leader is and then opens a second connection to whatever the broker's own `advertised.listeners` names. A broker the engine starts in a container cannot know the host-side port the orchestrator will allocate for it, so it can only advertise an address the host will not be able to reach, and the step's produce or consume then fails after the broker has answered — an environment error about a connection, not about the declaration. Point the suite at a broker whose advertised address is reachable from where the run executes; a `kafka` **dependency** has this handled for it, and is the shape to use unless the broker's own entrypoint and configuration are what the suite exists to exercise.
