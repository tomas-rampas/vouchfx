...
**Principle:** An `image:` field overrides the main container only (never a sidecar); a registry-qualified value is used exactly as written, and an un-qualified value is prefixed by `imageRegistry` when one is set, as §3.2.2 states. `imageRegistry` reaches every un-qualified image (including the Kafka schema-registry sidecar) but not images that carry a registry hostname of their own. `imagePullPolicy` applies to all containers.
