...
| 4 | Inconclusive — the aggregate verdict was Inconclusive and the author opted in, OR any parse failure occurred, OR an Inconclusive suite was refused before anything ran, OR any scenario was refused at a provider- or engine-surface guard, OR an unconfirmable `security:` declaration on that verdict. | Set `--fail-on-inconclusive` to exit 4 instead of 0 on timeout or unmet capture dependency. The other four reach 4 without it — but none overrides a code already chosen (the security rule is decided inside the verdict-to-exit mapping itself, independent of the flag; the other three fire only while the code so far is Success): a `Fail` still exits 1 and a gated environment error still 3. |
