...
- **The client key must be an unencrypted PEM, and both files must live inside the suite directory.** `clientCert` and `clientKey` are read as a PEM certificate and its matching PEM private key. There is no field for a key passphrase, so a key that is encrypted at rest — the form an enterprise PKI hands out by default — cannot be used, and neither can a PKCS#12/PFX bundle carrying the certificate and key together; the Kafka providers pass the same two paths to their client library, also with no passphrase. Export the key as an unencrypted PEM (a `PRIVATE KEY`, `RSA PRIVATE KEY` or `EC PRIVATE KEY` block, never `ENCRYPTED PRIVATE KEY`) before declaring it, and keep both files within the directory tree containing the `.e2e.yaml`: every path in this block must be written relative to that directory and must resolve inside it, so an absolute path is rejected by name even when it points at the file you meant. Note the asymmetry with `caCert`, which *is* format-tolerant — the trust anchor is loaded through a path constructor that auto-detects PEM and DER, while the client pair is PEM-only. A `clientKeyPassword` field could be added later without breaking any suite that validates today, so this is a boundary of the current release rather than a permanent one.
