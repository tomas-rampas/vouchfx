...
When a project declares multiple endpoints, the engine's selection depends on whether the service's `endpoint:` field is set. An optional **`endpoint:`** field names which of the discovered launch-profile endpoints to stage as `svc::<name>` — matched by name under case-sensitive Ordinal comparison against the endpoint names Aspire discovered (e.g. `http`, `https`, `http2`). An unmatched `endpoint:` value is refused at topology-build time, naming every endpoint the project declared; this check cannot run during `vouchfx validate` because a project's launch profile is not modelled at authoring time. Omit `endpoint:` and the engine applies its fixed rule: plaintext http where one exists, else https, else the first declared endpoint. Both paths share one consequence: selecting an https endpoint grants no client trust material and configures no security. A `project:`-form service cannot declare `security`, so the certificate is checked only against the host's own default trust store; if the host does not trust it, the request fails with an environment error, which exits the process with code 0 by default unless `--fail-on-env-error` is passed. Selecting an https endpoint is a **choice about which listener is addressed, not a security operation**. A terminal advisory fires when a targeted project-form service's staged address is an https endpoint, whether the author named it through `endpoint:` or the engine's fixed rule selected it. It names the service and the selected endpoint and states what the engine did not do: no client trust material is configured for that listener, so the certificate it presents is checked against the host's own default trust store and nothing else, and vouchfx asserts nothing about the outcome. The advisory reports the run's transport rather than an authoring fault — it is what a healthy run over an https project endpoint looks like.
...
**Three fields are `image:`-form only; one is `project:`-form only.** A `project:`-form service declaring `ports`, `healthCheck` or `httpPort` is rejected at schema-validation time, before anything starts, naming the offending field. Aspire discovers a project's endpoints from the project's own launch profile, so none of the three has anywhere to take effect there, and a field that does nothing must not silently validate. `httpPort` is the newest member of that list: earlier releases accepted it on a `project:`-form service and silently ignored it, which is exactly the failure mode the rejection closes — it pins a *port* rather than selecting a *listener*, so it was never the endpoint selector it resembles. The one field that goes the other way is `endpoint:`, which is `project:`-form only and is rejected on an `image:`-form service: an image-form service already controls what it exposes through `httpPort`, `ports` and `security.endpoint`, so `endpoint` has no meaning there. (A `project:`-form service likewise cannot be secured — §3.2.6b.)
