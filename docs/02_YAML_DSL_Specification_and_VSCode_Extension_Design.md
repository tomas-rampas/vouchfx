...
| **timeout** | No | An upper bound on how long the step may take, expressed as a duration such as 30s — enforced for every verify mode. An IMMEDIATE step that exceeds it resolves as **Inconclusive** (`step-timeout`, never Fail — §12.1): providers observe the step's cancellation cooperatively, and a body that completes past the bound has its outcome superseded. When declared, the value becomes the step's governing bound (it replaces the provider's built-in transport timeout, so a budget longer than that convention is honoured); when omitted, the provider's transport conventions (typically 30 seconds) are the de facto bound. For a RETRY step it bounds the polling window. |
