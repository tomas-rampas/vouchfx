...
| 4 | Inconclusive — the aggregate verdict was Inconclusive and the author opted in, OR one of the two no-verdict rules applied, OR any scenario was refused at a provider- or engine-surface guard. | Set `--fail-on-inconclusive` to exit 4 instead of 0 on timeout or unmet capture dependency; a parse failure, an Inconclusive suite refused before anything ran, or a scenario refused at a provider- or engine-surface guard, exits 4 without it. |
