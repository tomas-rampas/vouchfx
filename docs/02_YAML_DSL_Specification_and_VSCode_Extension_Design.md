...
**Project-form services and endpoint discovery.** A `project:`-form service that is targeted by a step must declare an `applicationUrl` in its launch profile (`launchSettings.json` on .NET projects), because that is where the engine discovers which endpoints the service exposes. A project-form service that no step targets is unaffected — a worker service with no HTTP endpoint is a legitimate shape and will still start without an application URL.

When a project declares multiple endpoints, the engine's selection depends on whether the service's `endpoint:` field is set. An optional **`endpoint:`** field names which of the discovered launch-profile endpoints to stage as `svc::<name>` — matched by name under case-sensitive Ordinal comparison against the endpoint names Aspire discovered (e.g. `http`, `https`, `http2`). An unmatched `endpoint:` value is refused at topology-build time, naming every endpoint the project declared; this check cannot run during `vouchfx validate` because a project's launch profile is not modelled at authoring time. Omit `endpoint:` and the engine applies its fixed rule: plaintext http where one exists, else https, else the first declared endpoint. Both paths share one consequence: selecting an https endpoint grants no client trust material and configures no security. A `project:`-form service cannot declare `security`, so the certificate is checked only against the host's own default trust store; if the host does not trust it, an HTTP request to that endpoint fails the handshake, which is classified an environment error and exits the process with code 0 by default unless `--fail-on-env-error` is passed. Selecting an https endpoint is a **choice about which listener is addressed, not a security operation**.

**Two transport advisories can appear in the terminal, and they report opposite choices.** The *downgrade* notice fires only where a targeted project-form service declares no `endpoint:` and the fixed rule therefore picked a plaintext listener over an https one the same project also declares; it names both endpoints and states that steps targeting the service will use plaintext. An explicit `endpoint:` selection is silent there, because the author made that choice knowingly. The *trust* notice fires whenever the staged address of a targeted project-form service is an https listener, however that endpoint was chosen — by `endpoint:` or by the fixed rule; it names the service and the selected endpoint and states what the engine did not do, which is to configure any client trust material for that listener: no trust anchor, no peer pinning, no client identity, and no assertion about the outcome. Each is printed once per affected service when the topology is built, and replayed unchanged on a `--watch` re-run that reuses that topology — a project's endpoints come from its launch profile, which is not part of the `environment` block, so editing one does not rebuild the topology. Each also reaches the JSON Lines event stream as a `transport-notice` record, one per advisory per topology build, matching what the terminal prints, so a CI job can read the advisory rather than scrape it out of console output. **That record's reach is `--events` and `--events-stream`, and no further:** the JUnit and HTML renderers ignore an event type they do not recognise, so a pipeline whose only artefacts are `--junit` and `--html` gets a green report with nothing in it about the transport. The replay is the terminal's alone — `--watch` is wired to no report artefact whatever, so a re-run prints the line again and writes no record anywhere. Both report the run's transport rather than an authoring fault: the trust notice in particular is what a healthy run over an https project endpoint looks like.
...
**`minio`** — vouchfx provisions a single container pinned to `bitnamilegacy/minio:2025.7.23-debian-12-r5`, a Bitnami build of MinIO 2025-07-23 on Docker Hub. That reference carries no registry hostname, so `imageRegistry` **does** apply to it: `imageRegistry: artifactory.mycompany.com/docker-mirror` redirects the pull to `artifactory.mycompany.com/docker-mirror/bitnamilegacy/minio:2025.7.23-debian-12-r5`, and the mirror must be able to serve that repository. The built-in pin moved away from upstream MinIO's own image because upstream withdrew its images from Docker Hub and quay.io entirely; the Bitnami repository is a frozen archive ("Legacy Bitnami images (no longer updated)", last push 2025-08-19) and a stopgap, with #581 tracking a maintained S3-compatible default. To pull the same build from another registry or repository, override it per-dependency with `image: artifactory.mycompany.com/bitnamilegacy/minio:2025.7.23-debian-12-r5`; `version:` will not help, because it only replaces the tag and cannot change the registry or repository.

**Principle:** An `image:` field overrides the main container only (never a sidecar); a registry-qualified value is used exactly as written, and an un-qualified value is prefixed by `imageRegistry` when one is set, as the paragraph above states. `imageRegistry` reaches every un-qualified image (including the Kafka schema-registry sidecar) but not images that carry a registry hostname of their own. `imagePullPolicy` applies to all containers.
...
**Three fields are `image:`-form only; one is `project:`-form only.** A `project:`-form service declaring `ports`, `healthCheck` or `httpPort` is rejected at schema-validation time, before anything starts, naming the offending field. Aspire discovers a project's endpoints from the project's own launch profile, so none of the three has anywhere to take effect there, and a field that does nothing must not silently validate. `httpPort` is refused rather than merely ignored because it is the member of that list an author is most likely to reach for by mistake: it *resembles* an endpoint selector and is not one, pinning a *port* where selecting a *listener* is the job of `endpoint:` — which its rejection message says, and names. That field goes the other way: `endpoint:` is `project:`-form only and is rejected on an `image:`-form service, which already controls what it exposes through `httpPort`, `ports` and `security.endpoint`, so `endpoint` has no meaning there. (A `project:`-form service likewise cannot be secured — §3.2.6b.)

**Pinning the host port.** Each entry takes one of two forms. A **bare integer** names the *container* port and leaves the host port to the orchestrator, which allocates a free one per run — the original behaviour, and the right default, because nothing outside the engine needs to know the number. A **`"<host>:<container>"` string** (docker-compose's ordering) additionally pins the host side: `ports: ["19093:9093"]` publishes container port 9093 on host port 19093. Quote it: an unquoted `19093:9093` is a mapping key to a YAML reader expecting a scalar, and quoting is also how every compose file writes it. Every port is a plain decimal integer with **no leading zero** — the same rule `security.endpoint` already carries, and for the same reason: the document is read twice, and a leading zero is octal to one of those readers and decimal to the other. The **host** half must be 1024–65535; ports below 1024 are privileged and well-known, and pinning one would squat a real service's port on this machine for the length of the run. The **container** half may be anything in 1–65535, because it lives in the container's own namespace. A malformed entry is rejected before anything starts.

**`httpPort` cannot be pinned**, and a container port that `ports` **pins** may not also be named by `httpPort` — the two would produce competing declarations of one port, and nothing decides which the pin belongs to. Pin through `ports` instead. (The unpinned overlap, `ports: [P]` alongside `httpPort: P`, is unchanged by this requirement and remains permitted.)

**What the editor can and cannot tell you.** The VS Code extension binds this same schema, so a malformed entry is flagged as you type — but the schema checks the *shape* of a pinned pair, not the numeric ranges: it cannot express "1024–65535 on the left, 1–65535 on the right" without an unreadable regular expression. So `"99999:99999"` and `"1023:9093"` draw no squiggle and are refused only when you run `vouchfx validate` or `vouchfx run`, with a message naming the value and the rule. A bare integer with a leading zero, `ports: [0123]`, escapes it for a different reason — the schema's `pattern` keyword applies to strings and is a no-op against an integer instance — and is likewise refused only by the parser.

Pin when — and only when — *the service's own configuration has to name the address its clients will reach it on*. The case this exists for is a broker's advertised listeners: a Kafka broker announces, in its own `env:`-supplied configuration at startup, the address clients should connect to for the partition leader. With an orchestrator-allocated host port there is no number the broker could announce that is true from outside its container, so a step bootstraps successfully over the staged endpoint, receives metadata pointing somewhere unreachable, and fails at the last hop. Everything else should stay on bare integers: pinning trades away the orchestrator's freedom to avoid a collision, and a suite that pins a port another process holds fails the run — see below.

Two consequences worth knowing before pinning. A pinned port cannot be substituted: if it is already in use the run stops before any container starts, with an **Environment error** naming the port and the declaring service, rather than quietly publishing on a different port (which would leave the broker announcing a number nothing listens on — the very failure pinning exists to remove). And pinning does not change how anything else in the language refers to the port: endpoint names (`tcp-<port>`), `security.endpoint`'s selector and `healthCheck.port` all continue to name the **container** port, which is the half that identifies the listener.
...
#### 3.2.6b Transport security: reaching a service or a Kafka broker over TLS or mutual TLS

> **A runnable example of everything in this section:** [`examples/security-mtls.e2e.yaml`](https://github.com/tomas-rampas/vouchfx/blob/main/examples/security-mtls.e2e.yaml) drives a client-certificate REST call and a mutual-TLS Kafka publish and consume against one topology, both chaining to one private certificate authority, with the certificates produced by a script that must run before vouchfx does. For which technologies can be reached over a secured connection at all, see the [security compatibility matrix](security-matrix.md).

A service may carry an optional **`security`** block declaring that its endpoint speaks TLS and — under `profile: mtls` — the client certificate the engine must present to it. A `kafka` dependency may carry the same block, on the same terms, and *Kafka targets* below states what differs there. No other dependency kind accepts it: on a `postgres`, `redis`, `mongodb`, … dependency the block is rejected outright at validation time, because this release wires a client connection for exactly those two target kinds. There is no profile and no substitute declaration that restores it for the others — transport security for the remaining dependency kinds is a later capability. Two fields are required whenever the block is present. `profile` is `tls` or `mtls`, matched case-sensitively and resolved against the engine's registry of wired profiles rather than by the schema alone. `endpoint` selects *which* of the service's ports is the secured one, and accepts either a bare port number (decimal, 1–65535, no leading zero) or the name of an endpoint the service's own `ports:`/`httpPort:` declaration produces (`http`, or `tcp-<port>`). Only an **explicit** `ports:`/`httpPort:` declaration produces a selectable name: a service declaring neither has no named endpoint to select, because the implicit `http` endpoint it would otherwise receive is suppressed by the `security` block itself (see the table below), so `endpoint` must be written as a port number there — `endpoint: http` on such a service matches nothing and is rejected with `Declared endpoints: (none)`. `endpoint` is mandatory rather than defaulted on purpose: a system under test that keeps a plaintext listener open beside a secured one would otherwise be reachable through an implicit default that quietly resolved to the wrong one.

The remaining fields are trust material. All but one of them are **paths**, resolved relative to the directory containing the `.e2e.yaml` file: `caCert` is optional under both profiles, and `clientCert` and `clientKey` are required together under `mtls` and forbidden under `tls`. The exception is **`clientKeyPassword`**, which is not a path at all but a `${secret:<source>/<path>}` reference to the passphrase protecting an encrypted `clientKey`; it is optional under `mtls` — whether the declared key is encrypted is a property of that file rather than of this document — and forbidden under `tls`, where there is no client key for a passphrase to unlock. Each declared path is checked for containment within the suite directory and then for existence at `vouchfx validate` time, before any container starts. A `${secret:…}` reference written into one of the path-valued fields — `caCert`, `clientCert`, `clientKey`, or a `serverArtifacts[].source` — is rejected there, naming the field and stating that it takes a path: the engine reads and copies the *file* such a field points at, while the secrets subsystem yields a *value*, which is not a file and has no path. An undeclared optional field is absent rather than missing — nothing is defaulted or synthesised in its place.
...
| **The form that endpoint resolves to** | The engine stages the endpoint in the form the steps addressing it actually consume, so no provider has to reshape it. A target addressed by the HTTP family resolves to an `https://host:port` URL. A target addressed by `mq-publish.kafka`/`mq-expect.kafka` resolves to a bare `host:port` bootstrap authority, carrying no scheme — the form a Kafka client expects. The engine reads which one from the suite's own steps, the same way it decides how far to confirm the endpoint before the run. Addressing one target with **both** families is rejected, naming the target: one endpoint stages one value, and the two forms are mutually exclusive. `vouchfx validate` reports it when a single scenario addresses one target with both families. When a multi-scenario suite splits the two families across its scenarios, the rejection comes from `vouchfx run` instead — before the topology is built, with the same message: those scenarios share one topology, so the staged form follows the union of the steps across every scenario the run will actually execute, which no per-file check can see. Nothing has started when the rejection arrives, on any of the three paths: `--watch` runs the same per-scenario check on every save, before it decides whether to reuse or rebuild a topology, so a conflicting save is refused with no container started on the first save and without touching the kept topology on a later one. Declare the broker and the HTTP API as two entries under `environment.services`. |
...
**Which hostname your server certificate must carry.** This is the single most common way a correctly-declared `security` block still fails, so read it before generating anything. The engine does not connect to the logical service name you declared. It connects to the **host-side endpoint the orchestrator allocates** for that service, which is a **loopback address** — `https://localhost:<allocated port>` in this release. The system under test's own server certificate must therefore name **`localhost`** in its subject alternative name, and naming **`127.0.0.1`** alongside it is the safe form, since both spell the same loopback host and only one of them is a DNS name. Issuing that certificate for `orders-api` — the logical name, and the only name this document otherwise puts in front of you — produces a hostname mismatch, which is **never** forgiven (a declared CA says which issuer to trust, not which host) and surfaces as an environment error rather than a test failure. The port is allocated at run time and is never part of the certificate.
...
**Three limits of the client half, all worth knowing before you design your PKI.** The two-tier support described above is a statement about the **server** direction, and a mutual-TLS reader will reasonably try to apply it in both. It does not carry over — and the client half additionally constrains the form the material itself may take.

- **The client certificate is presented as a single leaf.** `clientCert` supplies one certificate; the engine does not send a client chain alongside it, and there is no field for declaring one. A client certificate issued by an **intermediate** therefore authenticates only if the system under test can already obtain that intermediate itself — from its own truststore, or from material its author installed in the container. This is the mirror image of the server direction, where the peer sends its intermediate and the engine builds the path. If your enterprise CA issues client certificates from its issuing intermediate — the usual arrangement — either install that intermediate on the system under test, or issue the suite's client certificate directly from the anchor you declare. **Concatenating the chain into `clientCert` is not a workaround**: a PEM holding the leaf followed by its intermediate loads the leaf and silently discards everything after it, and the same file written the other way round fails to load at all, reporting that the key does not match the certificate.
- **The client pair is PEM-only and must live inside the suite directory; an encrypted key is declared with `clientKeyPassword`.** `clientCert` and `clientKey` are read as a PEM certificate and its matching PEM private key. A key that is encrypted at rest — the form an enterprise PKI hands out by default — is usable as it stands: leave it encrypted and declare its passphrase beside it as `clientKeyPassword: ${secret:env/CLIENT_KEY_PASS}`. What has no field, and so cannot be used, is a **PKCS#12/PFX bundle** carrying the certificate and key together; there is likewise no passphrase for `caCert`, a trust anchor being a public certificate with no private key to unlock. Keep both files within the directory tree containing the `.e2e.yaml`: every path in this block must be written relative to that directory and must resolve inside it, so an absolute path is rejected by name even when it points at the file you meant. Note the asymmetry with `caCert`, which *is* format-tolerant — the trust anchor is loaded through a path constructor that auto-detects PEM and DER, while the client pair is PEM-only.

  `clientKeyPassword` takes a whole `${secret:<source>/<path>}` reference and nothing else: a literal passphrase, a partly interpolated string, and a reference with text around it are all refused at authoring time, because a literal here would be a plaintext passphrase committed beside the encrypted key it unlocks. The reference is resolved at **first use of the certificate material** — inside the certificate load itself, once the topology is up, which in practice means the pre-run confirmation probe described below — and never at compile time, so no passphrase is baked into the compiled script and the reproducibility envelope hashes the reference rather than the value. `mq-publish.kafka` and `mq-expect.kafka` set their client library's `ssl.key.password` from the same resolved value, so a suite's Kafka legs open the same encrypted key its HTTP legs do, and `--watch` resolves the reference against exactly the sources `vouchfx run` does. Only the `ENCRYPTED PRIVATE KEY` (PKCS#8) form can be opened this way: a key written in openssl's legacy form, marked `Proc-Type: 4,ENCRYPTED`, must first be converted with `openssl pkcs8 -topk8`.

  Each contradiction around the field fails closed with a diagnostic rather than passing silently. A passphrase declared against a key that is **not** encrypted is refused, because a passphrase that decrypts nothing is a control that is not running — which is how a key rotated back to plaintext would otherwise go unnoticed. So is a passphrase declared with no `clientKey` for it to unlock, and a reference that resolves to an **empty** value. A *wrong* passphrase names `clientKeyPassword` as the likeliest cause — the other being an encryption form this runtime cannot read — and reports neither the value nor its length.
- **Whether a green `mtls` suite proves the *server demanded* an identity depends on the target.** Against a Kafka-speaking target the engine confirms it directly and reports `AuthenticatedRoundTrip` (see *Confirmation before the suite runs*, below). Against every other target the confirmation stops at `TransportConfirmed`: the engine presents the declared certificate, but a listener configured to *request* one without enforcing it is indistinguishable from an enforcing one, so a suite pointed at the former passes exactly as it would against the latter. Where mutual TLS being enforced is itself the thing under test and the target is not one the engine can round-trip, assert it at the system under test — for example, a step that expects the rejection an unauthenticated caller should receive.

**Kafka targets.** `mq-publish.kafka` and `mq-expect.kafka` configure their producer and consumer from the `security` block declared on the target they name — a `kafka` dependency, or a service standing in for a customer-supplied broker. `profile: tls` verifies the broker only; `profile: mtls` additionally presents the declared `clientCert`/`clientKey`. A declared `caCert` becomes the client's trust anchor; omitting it leaves the client's own default trust resolution untouched. A target that declares no `security` block is connected to in plaintext, exactly as before. Three limits are worth knowing. The engine does not secure a managed dependency's *own listener*: the block declares the client-side expectation for infrastructure whose server side the author has already configured — a custom `image:` for the dependency, with its keystore supplied through `serverArtifacts`. On a dependency, `endpoint` does not select a second endpoint the way it does on a service; the engine reaches the dependency at the bootstrap address the orchestrator publishes for it, so a secured listener on a container port other than the one the dependency's own resource exposes is not reachable at all, and fails closed naming both halves.

The third limit applies to a broker declared as a **service**, and it is a configuration requirement rather than a dead end. The engine stages that service's endpoint as a bootstrap authority a Kafka client can use directly, and its own pre-run confirmation reaches the broker over exactly that address. A *step*, however, does more than connect to the bootstrap: its client asks the broker where the partition leader is and then opens a second connection to whatever the broker's own `advertised.listeners` names. That last hop is reachable when — and only when — two declarations agree. **Pin the host port** the broker's container port publishes on (`ports: ["19093:9093"]`, §3.2.6a), and configure the broker's own `advertised.listeners` through its `env:` map to name that same pinned host port on the host address the run reaches it at (`localhost`). With both in place a service-form broker is produced to and consumed from exactly like a dependency.

Leave the port **unpinned** and the hop still fails, which is the trap to design around: a broker the engine starts in a container cannot know the host-side port the orchestrator will allocate for it, so there is no address it could advertise that the host will reach. The step then bootstraps successfully, receives metadata pointing somewhere unreachable, and fails after the broker has answered — an environment error about a connection, not about the declaration. This is the case §3.2.6a's own guidance says to pin for.

**Confirmation before the suite runs.** Once the topology is healthy — and before any seed, and therefore before any step — the engine connects to every declared endpoint itself, presenting the same material a step will, and refuses to run the suite if it cannot confirm the declaration. It is deliberately not a container health check: a health check cannot present a client certificate, so it can only show that something accepted a socket. Each target produces one declared-versus-observed line in the run's output, carrying a named level:

- **`AuthenticatedRoundTrip`** — the engine completed an application-protocol round trip over the secured connection. Today that means a Kafka `ApiVersions` exchange, and it is reached for any `kafka` dependency and for any target the suite's own `mq-publish.kafka`/`mq-expect.kafka` steps address, including one declared as a service. Under `profile: mtls` it additionally means a second connection presenting *no* client certificate was refused — so the broker both **required** and **accepted** the declared identity, and both halves are needed, because a peer that never asks for a certificate objects to nothing. Under `profile: tls` there is no client identity to accept and none is claimed. It says nothing about **authorisation**: whether that identity may publish to or consume from a particular topic is the broker's own per-request decision and surfaces as an ordinary step-level environment error.
- **`TransportConfirmed`** — the endpoint speaks TLS, its certificate satisfied the declared `caCert` (or the platform's own trust store where none is declared), and the declared client certificate was **presented**. Nothing here confirms the peer **accepted** it: the target's application protocol is not known from its declaration, and a completed TLS 1.3 handshake carries no such signal. Acceptance is first established when a step actually runs.

A `profile: tls` target that speaks Kafka therefore reports `AuthenticatedRoundTrip` while presenting no client certificate at all. That is accurate rather than a mislabelling: under `tls` it is the **server** that was authenticated — its certificate satisfied the declared `caCert` (or the platform's own trust store where none is declared), and that is the connection the round trip then ran over — so the level is a true statement about the peer and is merely silent about *whose* identity was confirmed. The rendered line resolves the two by adjacency, reading `client identity none declared, level AuthenticatedRoundTrip`.
...
A `mq-publish` step produces a message onto a broker. It is most often used to drive a test by injecting an event that the system under test is expected to react to. The mq-publish family has five Core providers: `mq-publish.kafka` (with full schema-registry and Avro support), `mq-publish.rabbitmq`, `mq-publish.nats`, `mq-publish.azureservicebus`, and `mq-publish.redis` (Redis Streams). The dotted form is always required; a bare `type: mq-publish` is not a valid step type. For `mq-publish.kafka` the transport follows the target: when the `kafka` dependency (or the service) the step names declares a `security` block (§3.2.6b), the producer connects over TLS, presenting the declared client certificate under `profile: mtls` and validating the broker against the declared `caCert`; a target that declares no block is connected to in plaintext exactly as before. The other four providers in this family address dependency kinds that accept no `security` block in this release.
...
| Field | Required | Meaning |
|---|---|---|
| target | Yes | Logical name of a declared **kafka** dependency to consume from, as declared under `environment.dependencies`, **or** a declared service (`environment.services`). Identical acceptance and connection-staging semantics to `mq-publish.kafka`'s own `target` row — see there for the full rationale. |
...
| `--changed-since <ref>` | Selects only scenarios whose file has changed since the given git reference (as determined by `git diff <ref>...HEAD` plus the dirty working tree). Each git invocation is bounded by a 2-minute process budget; exceeding it refuses selection rather than narrowing the file set to a partial capture, and exits 2. Three invocations are made independently (repository-root lookup, committed changes, and dirty working tree), so the worst case is about three times that duration. Requires a git repository. On Windows, git is located by an `.exe` candidate only, so a `.cmd`/`.bat` shim is not selected and is reported as not installed even though it is on `PATH` — a shim launches through `cmd.exe`, which reopens the argument-injection risk this resolution exists to avoid on a flag that splices an author-supplied ref into its own argv. |
...
| `--watch` | Run once, then watch the `.e2e.yaml` file and re-run automatically on save. Every save is validated first — the same authoring checks `vouchfx run` makes before it starts anything — so a save carrying an authoring fault is reported without starting a container and without disturbing a topology already running. Re-uses the already-built container topology while nothing the topology was built from has changed: the `environment` block, and the set of resource names the steps target. Editing a step's body, headers, assertions or URL path re-uses; adding a step that targets a service nothing addressed before, or changing which protocol family addresses one, rebuilds. Useful for fast local iteration on a single file. Press Ctrl-C to stop. Cannot be combined with `--parallel`. |
...
|---|---|---|
| 0 | Success — all scenarios passed, or only environment errors / inconclusive (the default, **only `Fail` breaks CI by default**). Four exceptions ignore the flags — see §16.4 of `docs/01`: any parse failure, an Inconclusive suite refused before anything ran, a scenario refused at a provider- or engine-surface guard (which applies even when a sibling ran), and an unconfirmable `security:` declaration. | Default. All scenarios Pass, or only EnvironmentError / Inconclusive and no opt-in flags set, and none of the four exceptions applies. |
| 1 | Test failure — at least one scenario failed (the test assertions did not hold). | Default; always exits 1 on Fail. |
| 2 | Usage error (bad arguments, missing directory, invalid `--changed-since` ref, a git invocation that exceeds its 2-minute budget, a git invocation whose output capture fails, or an unrecognised option). | The CLI's usage-error code. It also covers System.CommandLine parse errors (e.g. an unrecognised option): SCL itself returns exit 1 for those, and the CLI remaps that to 2 (see #269). |
| 3 | Environment error — the aggregate verdict was EnvironmentError and the author opted in, OR an unconfirmable `security:` declaration on that verdict. | Set `--fail-on-env-error` to exit 3 instead of 0 when infrastructure breaks (container fails to start, tunnel collapses, etc.); an unconfirmable `security:` declaration reaches 3 without it. |
| 4 | Inconclusive — the aggregate verdict was Inconclusive and the author opted in, OR any parse failure occurred, OR an Inconclusive suite was refused before anything ran, OR any scenario was refused at a provider- or engine-surface guard, OR an unconfirmable `security:` declaration on that verdict. | Set `--fail-on-inconclusive` to exit 4 instead of 0 on timeout or unmet capture dependency. The other four reach 4 without it (the security exception only when the aggregate verdict is Inconclusive; on an EnvironmentError verdict it exits 3) — but none overrides a code already chosen (the security rule is decided inside the verdict-to-exit mapping itself, independent of the flag; the other three fire only while the code so far is Success): a `Fail` still exits 1 and a gated environment error still 3. |

These rows describe `run` and `run --parallel`. **`--watch` applies no verdict-based exit gating at all** — it returns `UsageError` for a selection that is not exactly one parseable file, and `Success` otherwise, whatever the verdicts.
...
The same report appears in four places, fed from the same underlying event stream so that one place cannot drift from another.
...
| JUnit XML | Machine-readable output for CI systems that aggregate test results across many tools; not for human consumption. Each `<testcase>` `time` attribute is the scenario's wall-clock duration from its `scenario-started` to its `scenario-completed` event, including script compilation but not topology startup, rounded to the millisecond; the suite `time` is the sum of the scenario times. Topology startup and teardown fall outside every scenario, and under `--parallel` overlapping scenarios are each counted in full, so it is not the run's elapsed time and can be shorter or longer than it. |
