...
**Both fields are `image:`-form only.** A `project:`-form service declaring either `ports` or `healthCheck` is rejected at schema-validation time, before anything starts, naming the offending field. Aspire discovers a project's endpoints from the project's own launch profile, so neither field has anywhere to take effect there, and a field that does nothing must not silently validate. (A `project:`-form service likewise cannot be secured — §3.2.6b.)
...
A confirmation failure is an environment error that **breaks CI whether or not `--fail-on-env-error` was passed** (exit 3), and so does a `security` declaration the engine refuses before any container starts, whatever refused it (exit 4) — a certificate or artefact path that escapes the suite directory or does not exist, an artefact `target` that is not an absolute in-container file path, a profile with no wiring for the target's kind, a schema error *anywhere* in a document that declares `security` (nothing downstream of a schema rejection runs, so the declaration is never validated and therefore never confirmed), a `clientKeyPassword` that is not one whole `${secret:…}` reference naming a source the engine can resolve, or a multi-scenario suite whose scenarios resolve their declared security paths against different directories. Every *other* environment error still exits 0 by default. `docs/ci-integration.md` holds the full table and the maintained list of refusals — the set is open, so read it there rather than counting the causes named here.
