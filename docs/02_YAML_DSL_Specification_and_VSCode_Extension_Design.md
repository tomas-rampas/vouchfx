...
**`minio`** — vouchfx provisions a single container pinned to `quay.io/minio/minio:RELEASE.2025-09-07T16-13-09Z`. That reference embeds `quay.io`, so `imageRegistry` does **not** apply to it, for the same reason it does not apply to `azureservicebus`. This is a change: the built-in default was previously the unqualified `minio/minio`, which `imageRegistry` **did** redirect, and it moved because Docker Hub no longer serves that repository. If you rely on `imageRegistry` to route every pull through a mirror, note that this dependency now reaches `quay.io` directly unless you say otherwise — so a mirror's scanning, retention and provenance controls silently drop out of the path for it. Redirect it per-dependency with `image: artifactory.mycompany.com/minio/minio:RELEASE.2025-09-07T16-13-09Z`; `version:` will not help, because it only replaces the tag on the built-in repository and cannot change the registry.
