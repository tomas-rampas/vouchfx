...
- **A green `mtls` suite proves your client offered an identity, not that the server demanded one.** The engine presents the declared certificate and reports success on the response it gets back. It does not currently confirm that the listener actually *required* a client certificate, so a suite pointed at a listener configured to *request* one without enforcing it passes exactly as it would against a properly enforcing listener. If mutual TLS being enforced is the thing under test, assert it at the system under test — for example, a step that expects the rejection an unauthenticated caller should receive.
