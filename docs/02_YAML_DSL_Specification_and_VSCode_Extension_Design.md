...
| `--changed-since <ref>` | Selects only scenarios whose file has changed since the given git reference (as determined by `git diff <ref>...HEAD` plus the dirty working tree). Each git invocation is bounded by a 2-minute process budget; exceeding it refuses selection rather than narrowing the file set to a partial capture, and exits 2. Requires a git repository. |
