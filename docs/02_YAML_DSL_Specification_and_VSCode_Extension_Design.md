...
A confirmation failure is an environment error that **breaks CI whether or not `--fail-on-env-error` was passed** (exit 3), and so does a `security` declaration rejected before any container starts — a certificate path that escapes the suite directory or does not exist, or a profile with no wiring for the target's kind (exit 4). Every *other* environment error still exits 0 by default. See `docs/ci-integration.md` for the full table.
