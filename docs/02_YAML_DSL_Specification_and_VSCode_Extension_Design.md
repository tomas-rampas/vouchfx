...
| 4 | Inconclusive — the aggregate verdict was Inconclusive and the author opted in, OR any parse failure occurred, OR an Inconclusive suite was refused before anything ran, OR any scenario was refused at a provider- or engine-surface guard, OR an unconfirmable `security:` declaration on that verdict. | Set `--fail-on-inconclusive` to exit 4 instead of 0 on timeout or unmet capture dependency. The other four reach 4 without it — but each is a "never exits 0" rule conditioned on the code so far, so none overrides a code already chosen: a `Fail` still exits 1 and a gated environment error still 3. |
