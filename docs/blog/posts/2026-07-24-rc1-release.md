...
**All-unparseable suite exits 4 instead of 0** ([#278](https://github.com/tomas-rampas/vouchfx/issues/278)). Running over a directory where every scenario fails to parse now exits **4 (Inconclusive)** rather than **0**. Before, a completely broken suite looked green to CI. Now it matches the `validate` command behaviour, unconditionally — no need for `--fail-on-inconclusive`.
