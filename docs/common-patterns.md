...
| **Inconclusive** | Engine could not decide (timeout on a RETRY step, unmet capture, partition); or the run produced no verdict (every scenario failed to parse, or the suite was refused before execution). | Exit 0 by default; Exit 4 if `--fail-on-inconclusive` (unconditional when no verdict produced) |

By default, **only Fail breaks CI** — environment errors and inconclusive results exit 0. Two exceptions break CI regardless of the gating flags: (1) an unconfirmable `security:` declaration, and (2) a run that produced no verdict (parse failures, or pre-execution refusals). This distinction lets your CI system handle each outcome independently: fail the build on a product defect, page on-call for infrastructure breakage, and escalate inconclusive results to reliability engineering.
