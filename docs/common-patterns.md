...
By default, **only Fail breaks CI** — environment errors and inconclusive results exit 0. Three exceptions break CI regardless of the gating flags: (1) an unconfirmable `security:` declaration; (2) any parse failure, or an Inconclusive suite refused before any scenario executed; and (3) any scenario refused at a provider- or engine-surface guard — a fault in the testing machinery rather than in your suite, identified by where it arose rather than by whose it is — which applies even when a sibling scenario ran and passed. A run that executed nothing but carries an `EnvironmentError` — a topology that never came up — is not the second case and still exits 0 by default, provided that is its only fault; if a sibling's provider had already thrown, the third case catches it and the run exits 4 by default, or 3 with `--fail-on-env-error`. This distinction lets your CI system handle each outcome independently: fail the build on a product defect, page on-call for infrastructure breakage, and escalate inconclusive results to reliability engineering.
