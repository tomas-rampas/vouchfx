...
- `unanalysedSourceFiles` maps a lower-case extension, dot included, to the number of files under the root that no `--exclude` glob matches, the defaults included, and that are in a source language the extractor does not read (`.py`, `.java`, `.kt`, `.js`, `.ts`, `.go`, `.fs`, `.vb` and similar). `--sources` does not enter into it: it chooses which C# files are read, and a file in another language never matches it. It does not affect `complete`, because the extractor never claimed those languages; the MCP reads it separately, to stay quiet in polyglot trees.
