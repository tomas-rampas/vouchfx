...
The proposed resolution for (b) is an optional `resourceRole` field on `environment-error`, taking the value `service`, `dependency` or `engine` and omitted when unknown. It carries names and roles only, never images, `env` values or connection strings. The field is **not** a free addition. `environment-error` is a frozen v1 record, and §14.4.2 treats a property added to a frozen record as a conscious, reviewed change: the `EventContractFreezeTests` golden moves and the wire contract increments. Renderers tolerate unknown fields, so no consumer breaks, but that tolerance is what makes the change safe, not what permits it. It therefore needs an explicit wire-contract decision by the maintainer. The freeze-neutral alternative is a new record type (one `resource` record per declared resource, carrying its name and role), which the gate's completeness test admits once it is listed in the frozen set; it costs a second record where one field would do, which is why this record recommends the field, subject to that decision. Either way it takes about a day in each repository and does not need to wait for U1.
...
- **Routes** (`kind: route`, `technology: aspnetcore`, `direction: serve`).
  ...
  - *Controllers:* a class-level `[Route]` combined with `[HttpGet]` … `[HttpOptions]` or an action-level `[Route]`. `[controller]` and `[action]` tokens are replaced from the class and method names, and a template starting with `/` or `~/` overrides the class prefix. No sample uses controllers. They are in scope because attribute arguments must be compile-time constants, so an attribute-routed template is always a literal or a named constant. The constant table below resolves it whenever the constant is declared in the analysed tree.
...
**Deterministic.** Entries are sorted ordinally by kind, method, name (or route, or pattern), file, line and column. The document carries no timestamps and no host paths, so a given tree always yields the same document, whatever the operating system and whatever order the file system listed it in. Only `engineVersion` varies, between engine releases.
...
2. **Parse on a dedicated thread with an explicit stack size.** The crash threshold follows the stack size, which differs by host: 20,000 nested parentheses kill a 1 MiB thread and the 8 MiB Linux main thread alike, but parse on a 64 MiB one. A fixed stack gives the caps the same safety margin on every host.
3. **Check the time budget between files.** `ParseText` ignores its token while parsing, so the caps bound a single file's cost, not the token, and the budget can overrun by at most one file.
