...
The proposed resolution for (b) is an optional `resourceRole` field on `environment-error`, taking the value `service`, `dependency` or `engine` and omitted when unknown. It carries names and roles only, never images, `env` values or connection strings. The field is additive under the frozen v1 event contract, because renderers already tolerate unknown fields. It takes about a day in each repository and does not need to wait for U1.
...
- **Preprocessor.** A file with `#if` directives is parsed twice: once with no symbols defined, and once with every symbol its conditions mention. The union is reported, and entries from conditional regions are marked `conditional: true`. A route behind `#if DEBUG` is therefore not lost.
...
- **Containment.** Enumeration never follows symbolic links or junctions: an entry carrying `FileAttributes.ReparsePoint` is neither descended into nor read. Each such entry under an included path is listed in `skipped` and makes the scan incomplete. Nothing outside the root is opened, and files are opened read-only. Nothing is written except `--output`, whose parent directory must exist, as for `plan --output`. There is no cache.
- **Exit codes.**
