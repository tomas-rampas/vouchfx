...
- **Constant folding.** The extractor folds:
  ...
  - `const string` fields and locals, anywhere in the analysed tree. A first pass collects them into a table keyed by scope, not by bare name: a local by its enclosing method or block, and a field by its containing type's full name (its namespace and any enclosing types). A reference is resolved the way C# looks it up, as far as syntax can follow: first a local in an enclosing block, then a field of the enclosing type and of each type around it, then a qualified name such as `Topics.Orders`, matched against the declared types that the file's namespace and `using` directives make visible. So two unrelated `Topic` constants in different classes never collide. A reference stays `unresolved`, with reason `ambiguous-constant`, only when more than one visible candidate with a different value remains. In code that compiles, that happens only where syntax cannot follow the lookup, such as a member inherited from a base type.
...
- for an entry the walk skipped (`too-large`, `symlink`, `hard-link`, `not-regular` or `unreadable`), the reason;
