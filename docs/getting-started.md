...
This distinction is crucial: a `Fail` means your code is broken; an `EnvironmentError` means your CI infrastructure is broken. By default, only `Fail` breaks the build — allowing you to distinguish product defects from infrastructure problems. You can opt in to gating on the others with `--fail-on-env-error` and `--fail-on-inconclusive`. Four
narrow cases break CI whatever those flags say — a parse failure, a suite refused before any scenario
ran, a scenario refused at a provider- or engine-surface guard, and a `security:` declaration the
engine could not confirm; see [CI integration](ci-integration.md) for each. Note: any parse failure — a malformed file, one the runner cannot read — never lets the run exit 0, independently of the `--fail-on-inconclusive` flag, matching the behaviour of `vouchfx validate`. It exits 4 unless another rule has already chosen a non-zero code: a failing scenario takes the run to 1, and a gated environment error to 3. One unreadable file beside a suite that otherwise passes is enough; the engine cannot say what that file would have asserted. The other three named above behave the same way — each never lets the run exit 0, and none overrides a code another rule already chose. [CI integration](ci-integration.md) documents all four and the exit code each cause carries.
...
When enabled, the engine monitors its standard input stream and gracefully initiates shutdown (equivalent to Ctrl+C) when the stream closes **or becomes unreadable** — the watcher treats a read failure exactly as it treats end-of-stream, and the run says so rather than claiming end-of-stream specifically. This allows the engine's own container and topology teardown to run to completion before the process exits, preventing orphaned containers. This flag is **opt-in and off by default** — normal interactive and CI runs are completely unaffected. If graceful shutdown does not complete within the teardown budget (approximately 30 seconds), the engine force-exits itself, making the flag self-safe — a host that closes stdin is guaranteed termination without needing to send a kill signal. A forced exit always yields exit code 4 (Inconclusive), independent of the `--fail-on-inconclusive` flag.
...
- `tier` (`"core"` or `"community"`; every type `vouchfx list` reports is `"core"`, because the CLI ships only Core providers). It is `null` only when an in-process host builds the catalogue without saying which providers are Core — the engine never guesses a tier from a name.
- `supportedVerifyModes` (the `verifyMode` values the type accepts, as wire tokens: `["IMMEDIATE", "RETRY"]` for every type, since the engine owns the `RETRY` polling loop for all of them)
- `docsUrl` (a link to the type's section of the [language reference](language-reference.md), e.g. `https://vouchfx.io/language-reference/#httprest`; `null` for a type that is not Core, which the reference does not document, and — like `tier` — `null` for every entry when the catalogue is built without a Core set at all)
- `example` (a complete, minimal `.e2e.yaml` suite exercising one step of the type, as a YAML string; `null` for a type that is not Core, and — like `tier` and `docsUrl` — `null` for every entry when the catalogue is built without a Core set at all). It is exactly what `vouchfx scaffold` produces for that step, without the provenance comments, over the smallest environment the step needs: one service named `app` for the `http` and `metrics-assert` families, one dependency named after its kind (`postgres`, `kafka`, `mailpit`, …) for a type that targets a dependency, and none for `script.csharp`, `trace-expect.otlp` or `webhook-listen.http`, which target nothing declared. A Core type can also get `null`, when its step declares a `target` that this environment cannot supply: the engine withholds an example it knows would fail rather than publish it. No shipped Core type hits this today, because each one either has no `target` or targets a dependency the scaffolder can name after its kind; a future type whose family maps to no known dependency kind would. Every example passes `vouchfx validate`. The values are placeholders that validate, not a meaningful test; replace them before trusting the result. Because dependencies are named after their kind and step ids after their type, examples can be merged: those for `mq-publish.kafka` and `mq-expect.kafka` share one `kafka` dependency.

There is deliberately no `vouched` field. The Vouched badge is an endorsement of a specific version of a Community provider, awarded by maintainer review and recorded in the [provider hub](https://github.com/tomas-rampas/vouchfx-providers)'s registry, which the engine does not read; Core providers are not subject to it. A tool that wants to show it combines `tier` with the hub's registry.

This shape is the contract for integration with tooling such as [vouchfx-mcp](https://vouchfx-mcp.vouchfx.io/) and the VS Code extension. Incomplete metadata fails the entire export rather than omitting fields silently. Within `schemaVersion` 1 the document only ever gains fields, so a consumer should ignore members it does not recognise.
...
- `EngineExport.BuildCatalogue(StepKindRegistry, engineVersion, coreProviderAssemblies)` — the same document with `tier` stated for every entry, `docsUrl` filled for each Core entry, and `example` filled for each Core entry whose step target the canonical environment can derive (`null` for a Core entry it cannot — see "Listing step types" above); a provider is Core when its type is declared in one of the assemblies you pass. The overload without the set leaves those three fields `null`.
