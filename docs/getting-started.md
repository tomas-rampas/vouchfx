...
- `docsUrl` (a link to the type's section of the [language reference](language-reference.md), e.g. `https://vouchfx.io/language-reference/#httprest`; `null` for a type that is not Core, which the reference does not document)
- `example` (a complete, minimal `.e2e.yaml` suite exercising one step of the type, as a YAML string; `null` for a type that is not Core). It is exactly what `vouchfx scaffold` produces for that step, without the provenance comments, over the smallest environment the step needs: one service named `app` for the `http` and `metrics-assert` families, one dependency named after its kind (`postgres`, `kafka`, `mailpit`, …) for a type that targets a dependency, and none for `script.csharp`, `trace-expect.otlp` or `webhook-listen.http`, which target nothing declared. Every example passes `vouchfx validate`. The values are placeholders that validate, not a meaningful test; replace them before trusting the result. Because dependencies are named after their kind and step ids after their type, examples can be merged: those for `mq-publish.kafka` and `mq-expect.kafka` share one `kafka` dependency.
...
- `EngineExport.BuildCatalogue(StepKindRegistry, engineVersion, coreProviderAssemblies)` — the same document with `tier` stated for every entry, and `docsUrl` and `example` filled for each Core entry; a provider is Core when its type is declared in one of the assemblies you pass. The overload without the set leaves those three fields `null`.
