...
The `--events-stream` flag writes the same schema-versioned JSON Lines event records as the buffered `--events` archive, but incrementally as the run proceeds to a tailable file. The file is opened with a shared read/write handle (UTF-8 without BOM), allowing external tools to tail it in real time — useful for:
...
- **Tailing requires shared access.** A tool reading from the file whilst the engine writes must open it with shared read/write access (on Windows, `FileShare.ReadWrite`; on Unix, the file is readable immediately as the engine holds the write handle).
