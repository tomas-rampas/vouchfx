...
| 4 | Inconclusive (timeout, unmet captures; or the run produced no verdict) | Only if `fail-on-inconclusive: true` — but a run that produced no verdict always exits 4 |
