...
**`minio`** — vouchfx provisions a single container pinned to `bitnamilegacy/minio:2025.7.23-debian-12-r5`, a Bitnami build of MinIO 2025-07-23 on Docker Hub. That reference carries no registry hostname, so `imageRegistry` **does** apply to it, as it does to every other Docker Hub dependency. The built-in pin moved away from upstream MinIO's own image because upstream withdrew its images from Docker Hub and quay.io entirely; the Bitnami repository is a frozen archive ("Legacy Bitnami images (no longer updated)", last push 2025-08-19), so the build is older than the previous pin and receives no security fixes, and #581 tracks a maintained S3-compatible default. A suite that must hold a specific digest can pin one per dependency with `image: bitnamilegacy/minio@sha256:6dabb4a2088c9a79908de3bc05f4586c23ad2182c8908e7e3acbf61c1467fb20`, which the engine uses as written, prefixed by `imageRegistry` when one is set, like any other unqualified `image:`.
