...
#  whichever tool is already there.  Both write exactly the same seven files.
#
...
command -v openssl >/dev/null 2>&1 || {
  ...
  printf '       On Windows run examples/security-mtls.setup.ps1 instead — it uses\n' >&2
  printf '       .NET, which PowerShell 7 already carries.\n' >&2
  exit 1
}
...
openssl req -x509 -newkey rsa:2048 -nodes \
  -keyout "${work}/ca-key.pem" -out "${work}/ca.pem" -days "$DAYS" \
  -subj "/CN=${CA_SUBJECT}" \
...
openssl req -newkey rsa:2048 -nodes \
  -keyout "${work}/server-key.pem" -out "${work}/server.csr" \
...
# authenticated.
openssl req -newkey rsa:2048 -nodes \
  -keyout "${work}/client-key.pem" -out "${work}/client.csr" \
...
# live path. The intermediate CSRs and the CA's own private key stay in the staging
# directory and are removed with it by the trap — the CA key in particular must not
# survive: nothing needs it once the two leaves are signed.
chmod 600 \
  "${work}/client-key.pem" \
  "${work}/server-key.pem" \
  "${work}/kafka.keystore.pem"

rm -f -- "${work}/ca-key.pem" "${work}/server.csr" "${work}/client.csr" "${work}/ca.srl"
