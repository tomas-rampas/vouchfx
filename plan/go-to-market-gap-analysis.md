...
| 6 | GTM-02 | NuGet: package IDs unreserved, no Platform.Sdk publish channel | Blocker | RE | Reserve both IDs after GTM-29 resolves; add Platform.Sdk pack/push to release.yml before v1.0 |
...
| GTM-02 | NuGet channel partially absent: `vouchfx` and `Platform.Sdk` IDs unreserved (BlobNotFound live, 2026-07-04); publishing is gated by NuGet.org Trusted Publishing policy created 2026-07-05 (no `NUGET_API_KEY` secret required); release.yml packs only the CLI nupkg — **Platform.Sdk has no publish channel at all**, yet vouchfx-providers depends on it via a local feed with one "REMOVE AFTER SDK PUBLISHED" marker; `Platform.Sdk` is a generic ID with prefix-reservation/squatting risk | Blocker | RE | api.nuget.org BlobNotFound for both IDs; Trusted Publishing policy active (7-day pending-activation window, permanent after first successful publish); `src/Cli/Vouchfx.Cli/Vouchfx.Cli.csproj:35-58` (PackAsTool, PackageId=vouchfx, Version 1.0.0); vouchfx-providers local-feed bootstrap | After GTM-29: create the nuget.org account under the entity/name, reserve both IDs (consider prefix reservation), add a Platform.Sdk pack/push job to release.yml before v1.0 | Open |
