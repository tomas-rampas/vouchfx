...
Plus: engine-owned RETRY polling (you never write `Thread.Sleep`), `${secret:…}` references resolved at execution time and redacted at the source, a four-verdict taxonomy where **only `Fail` breaks CI by default** (environment errors and timeouts are kept distinct, with four narrow exceptions that always break CI — a parse failure, a suite refused before anything ran, a provider- or engine-surface refusal, and an unconfirmable `security:` declaration), scenario selection by tag/owner/path/git-change-set, parallel topology-per-scenario runs, watch mode, and reporting as a schema-versioned JSON Lines event stream rendered to terminal, self-contained HTML and JUnit XML. Also includes `vouchfx plan` for deterministic coverage-and-gap analysis of your declared suites against run history and available providers; `vouchfx scaffold` to generate test skeletons from structured intent; and `vouchfx validate`, `list`, and `schema` for Docker-free compile-time tooling.
