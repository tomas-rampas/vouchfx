// Vouchfx.Engine.Compilation — ReservedNamespaceGuard (§5.6).
//
// Detects customer DLLs that squat on the reserved namespaces Vouchfx.Engine.*
// or Vouchfx.Steps.* and refuses them at suite start with a clear diagnostic.
//
...
/// <c>Vouchfx.Engine.*</c> or <c>Vouchfx.Steps.*</c> (§5.6).
/// </summary>
/// <remarks>
/// <para>
...
/// <remarks>
/// <para>
/// Reserved namespaces are <c>Vouchfx.Engine.*</c> (engine internals) and
/// <c>Vouchfx.Steps.*</c> (provider contract types).  Customer DLLs that declare
...
/// assemblies whose own simple name starts with <c>Vouchfx.Engine.</c> or
/// <c>Vouchfx.Steps.</c>, plus <c>Vouchfx.Sdk</c>.  An assembly that impersonates
/// a <c>Vouchfx.*</c> name is an assembly-name collision, handled by
...
public static class ReservedNamespaceGuard
{
    ...
    public static readonly IReadOnlyList<string> ReservedPrefixes =
        new[] { "Vouchfx.Engine.", "Vouchfx.Steps." };
        ...
        IReadOnlySet<string> trustedAssemblySimpleNames)
    {
        ...
        foreach (var assembly in assemblies)
        {
            ...
            // engine / provider assemblies that own Vouchfx.Engine.* / Vouchfx.Steps.*.
