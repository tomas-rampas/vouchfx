// Vouchfx.Steps.DbAssert.Mysql — db-assert.mysql step provider (DSL §5, §13.10).
//
// MySQL analogue of db-assert.postgres.  Same intent and model shape;
// different client (MySqlConnector instead of Npgsql).
//
...
/// <summary>
/// Core provider for the <c>db-assert.mysql</c> step kind (DSL §5).
/// Executes a parameterised SQL query against a declared MySQL dependency
...
/// CSX executes a parameterised MySqlConnector query, evaluates row-count and/or column-value
...
/// </remarks>
[StepProvider]
public sealed class DbAssertMysqlProvider
    : IStepProvider,
      IStepBinder<DbAssertMysqlModel>,
      IStepValidator<DbAssertMysqlModel>,
      IStepCompiler<DbAssertMysqlModel>,
      IResourceContributor<DbAssertMysqlModel>,
      ...
      IStepDiffRenderer
{
    ...
    /// <inheritdoc />
    public StepKindId Kind { get; } = new StepKindId("db-assert", "mysql");
    ...
    /// <summary>
    /// Gets the JSON Schema fragment that describes the <c>db-assert.mysql</c>
    ...
    public JsonSchemaFragment SchemaFragment { get; } = new JsonSchemaFragment(
        """
        {
          "description": "Runs a parameterised SQL query against a MySQL dependency and asserts on the returned row count and/or first row's column values.",
          "type": "object",
          "required": ["target", "query", "expect"],
          "properties": {
            "target": {
              "description": "Logical name of the mysql dependency to query, as declared under environment.dependencies.",
              "type": "string",
              "minLength": 1
            },
            "query": {
    ...
    /// <inheritdoc />
    public DbAssertMysqlModel Bind(YamlNode node, IBindingContext ctx)
    {
    ...
    /// <inheritdoc />
    public ValidationResult Validate(DbAssertMysqlModel model, IProjectContext ctx)
    {
        var errors = new List<string>();

        // (a) target must not be empty.
        if (string.IsNullOrWhiteSpace(model.Target))
            errors.Add("db-assert.mysql: 'target' must not be empty.");

        // (b) query must not be empty.
        if (string.IsNullOrWhiteSpace(model.Query))
            errors.Add("db-assert.mysql: 'query' must not be empty.");
        ...
        {
            errors.Add(
                "db-assert.mysql: 'expect' must specify rowCount and/or row.");
        }

        // (d) dependency reconciliation: target must name a declared mysql dependency.
        if (!string.IsNullOrWhiteSpace(model.Target))
        {
    ...
    /// <para>
    /// The class name begins with <c>DbAssertMysql_</c> to prevent collisions when
    ...
    {
        "static class DbAssertMysql_Helpers\n" +
        "{\n" +
        "    /// <summary>\n" +
        "    /// Executes a parameterised SQL query via MySqlConnector, evaluates\n" +
        ...
        "        // their error verdict (this is the canonical pattern for db-assert.mysql).\n" +
        ...
        "        {\n" +
        "            // MySqlConnector-specific exception: network failure, auth error, etc. = EnvironmentError (§12.1).\n" +
    ...
    /// <remarks>
    /// <para>
    /// Emits a CSX block whose execution opens a MySqlConnector connection keyed by
    /// <c>VarKeys.Connection(model.Target)</c>, runs <c>model.Query</c> with the
    /// declared parameters bound as <c>MySqlParameter</c> instances via
    ...
    ///   <item><see cref="CsxFragment.RequiredHelpers"/> — full <c>static class DbAssertMysql_Helpers</c> definition; byte-identical across instances.</item>
    ...
    /// </para>
    /// </remarks>
    public CsxFragment Emit(DbAssertMysqlModel model, ICompileContext ctx)
    {
        ...
        var block = $$"""
            {
                await DbAssertMysql_Helpers.ExecuteAsync(
                    Vars,
    ...
    /// <c>Family="mysql"</c> and <c>Name=model.Target</c>.
    /// </remarks>
    public IEnumerable<ResourceRequirement> Resources(DbAssertMysqlModel model)
    {
    ...
    /// <inheritdoc />
    /// <remarks>
    /// Returns the <c>MySqlConnector</c> assembly so the Roslyn compiler
    /// can resolve <c>MySqlConnection</c> and related types in the emitted helper class.
    ...
    public IEnumerable<System.Reflection.Assembly> CompileReferenceAssemblies
    {
        get
        {
            yield return typeof(MySqlConnector.MySqlConnection).Assembly;
        }
    }
    ...
    /// <c>db-assert.mysql</c> Fail-observation shapes that this provider can render
    /// as an expected-vs-observed diff.
    /// </summary>
    /// <remarks>
    /// Recognised shapes (emitted by <c>DbAssertMysql_Helpers</c> on a Fail verdict):
    /// <list type="bullet">
