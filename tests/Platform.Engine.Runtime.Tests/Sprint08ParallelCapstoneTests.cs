...
public sealed class Sprint08ParallelCapstoneTests
{
    ...
    private static Platform.Engine.Authoring.Ast.ScenarioAst BuildAst(string yaml)
    {
        var registry = StepKindRegistry.BuildAndFreeze(s_providerAssemblies);
        var doc = Platform.Engine.Authoring.YamlDocumentParser.Parse(yaml);
        return Platform.Engine.Authoring.AstBuilder.Build(doc, registry);
    }
}
