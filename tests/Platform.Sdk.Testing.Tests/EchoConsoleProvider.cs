// Platform.Sdk.Testing.Tests — an in-test worked-example provider (echo.console).
//
...
// `Harness.Steps.Echo`.  `Platform.Steps.*` and `Platform.Engine.*` are RESERVED for the
// engine and its Core providers — a customer DLL declaring them is refused at startup.
//
// §5 MEMORY MODEL: this provider takes NO reference to Platform.Engine.Abstractions.  The
...
// the engine already references Platform.Engine.Abstractions when it compiles the
...
/// validate <see cref="Platform.Sdk.Testing.ProviderTestHarness"/> against a provider
