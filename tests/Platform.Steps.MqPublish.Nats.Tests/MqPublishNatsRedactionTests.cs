// §17 credential-redaction regression tests for MqPublishNatsProvider (non-docker).
//
// Directly exercises the emitted MqPublishNats_Helpers.RedactNatsUrl method by
...
namespace Platform.Steps.MqPublish.Nats.Tests;

/// <summary>
/// Non-docker credential-redaction regression tests for the emitted
/// <c>MqPublishNats_Helpers.RedactNatsUrl</c> method.
/// </summary>
public sealed class MqPublishNatsRedactionTests
{
    ...
    // Both NATS assemblies must be in the reference list so the emitted helper
    // (which references NatsConnection and NatsJSContext types) compiles.
    ...
    private static MqPublishNatsModel GetModel(
        string target = "nats-bus",
        string subject = "orders.created",
        string payload = "hello") =>
        new MqPublishNatsModel(target, subject, null, payload);
        ...
        string natsUrl, string craftedMessage, string stepId)
    {
        ...
        const string scriptBody =
            "Vars[\"__redact_result__\"] = MqPublishNats_Helpers.RedactNatsUrl(" +
    ...
    public async Task RedactNatsUrl_WithCraftedMessageContainingSecret_SecretIsStripped()
    {
        ...
        var result = await CompileAndRedactAsync(natsUrl, craftedMessage, "redact-publish-direct");
    ...
    public async Task RedactNatsUrl_TlsScheme_SecretIsStripped()
    {
        ...
        var result = await CompileAndRedactAsync(natsUrl, craftedMessage, "redact-publish-tls");
    ...
    /// step (b) strips the entire userinfo string, including empty username.
    /// </summary>
    [Fact]
    public async Task RedactNatsUrl_PasswordOnly_SecretIsStripped()
    {
        ...
        var result = await CompileAndRedactAsync(natsUrl, craftedMessage, "redact-publish-passonly");
    ...
    /// <summary>
    /// A NATS URL with no credentials (<c>nats://host:4222</c>) in a message that
    /// does not echo the URL is returned unchanged (no false positives).
    /// </summary>
    [Fact]
    public async Task RedactNatsUrl_NoCredentials_MessageReturnedUnchanged()
    {
        ...
        var result = await CompileAndRedactAsync(natsUrl, craftedMessage, "redact-publish-nocreds");
