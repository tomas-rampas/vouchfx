...
//   • Parameter DEFAULT VALUES are deliberately NOT captured (FormatParameters emits
//     type + name only): adding or removing a parameter default is not a binary-breaking
//     change, so a freeze gate must not flag it as contract drift.
using System;
...
public static class SdkPublicApiSignature
{
    ...
    /// </summary>
    /// <param name="assembly">The assembly whose public surface is canonicalised.</param>
    /// <param name="bannerTitle">
    /// The banner-line-1 title prefixed to the signature.  Each golden self-describes
    /// accurately via its own title: the <c>Platform.Sdk</c> golden passes the FROZEN v1
    /// provider-contract title; the <c>Platform.Sdk.Testing</c> golden passes its own
    /// versioned-testing-surface title (the test harness is versioned-not-frozen, NOT the
    /// frozen v1 provider contract).  Line 2 — the "Generated by the shared
    /// SdkPublicApiSignature canonicaliser; …" provenance note — is shared and unchanged.
    /// </param>
    public static string Build(Assembly assembly, string bannerTitle)
    {
