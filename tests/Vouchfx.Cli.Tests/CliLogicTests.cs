...
//   • ProviderRegistryFactory — names the 5 Core provider assemblies; the registry freezes
//     with the 5 expected step kinds.
...
{
    [Fact]
    public void CoreProviderAssemblies_ReturnsFiveDistinctAssemblies()
    {
        var assemblies = ProviderRegistryFactory.CoreProviderAssemblies();

        Assert.Equal(5, assemblies.Length);
        // Five *distinct* assemblies (no accidental duplicate anchor).
        Assert.Equal(5, assemblies.Distinct().Count());
    }

    [Fact]
    public void BuildCoreRegistry_FreezesWithTheFiveCoreStepKinds()
    {
