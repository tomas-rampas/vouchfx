...
public sealed class CollectibleResolutionTests : IDisposable
{
    ...
    /// <summary>
    /// Passes a path whose simple name ("Vouchfx.Engine.Abstractions") is already
    ...
    {
        // Use Vouchfx.Engine.Abstractions — it is always loaded in Default by this point.
        var abstractionsAssembly = typeof(ScriptGlobalVariables).Assembly;
        var abstractionsName = abstractionsAssembly.GetName().Name!; // "Vouchfx.Engine.Abstractions"
        var abstractionsPath = abstractionsAssembly.Location;

        Assert.False(string.IsNullOrEmpty(abstractionsPath),
            "Vouchfx.Engine.Abstractions.Location is empty — cannot test pin-avoidance.");
