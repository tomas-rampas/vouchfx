...
public sealed class RetryAssemblyTests
{
    ...
    public void Assemble_RetryPlan_WrapShape_ContainsPollAsyncAndNoUsingVar()
    {
        var fragment = StubFragment("""
            {
                Vars["__outcome::poll_step"] = new Vouchfx.Engine.Abstractions.StepOutcome(
                    Vouchfx.Engine.Abstractions.Verdict.Pass, 1L, null);
            }
            """);
    ...
    /// already-referenced <c>Vouchfx.Engine.Abstractions</c> assembly.  No Docker.
    /// </summary>
    [Fact]
        ...
        var fragment = StubFragment(""""
            {
                ...
                Vars["__outcome::poll_step"] = new Vouchfx.Engine.Abstractions.StepOutcome(
                    Vouchfx.Engine.Abstractions.Verdict.Pass, 0L, j);
            }
            """");
    ...
    public async Task Assemble_RetryPlan_NeverSatisfied_ResolvesToInconclusive()
    {
        var fragment = StubFragment("""
            {
                Vars["__outcome::poll_step"] = new Vouchfx.Engine.Abstractions.StepOutcome(
                    Vouchfx.Engine.Abstractions.Verdict.Fail, 0L, null);
            }
            """);
