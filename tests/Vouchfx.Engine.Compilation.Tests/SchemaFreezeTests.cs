...
//
//   NEVER PASS --no-build, TO REGENERATE OR TO VERIFY. The order is: regen, BUILD,
//   verify. Both directions bite, and one of them is silent:
//     • Verifying with --no-build after a regen compares the composed schema against
//       the STALE golden copy in bin/ — the regen writes the source file, and only a
//       build copies it. Symptom: a drift failure naming text you just wrote. Noisy,
//       and merely confusing.
//     • Regenerating with --no-build composes from the STALE EMBEDDED schema in the
//       already-built Compilation DLL, so an edit to root-language-schema.json is not
//       in it: the run writes the OLD schema over the source golden and exits 0.
//       Symptom: none. A green regen that silently reverts a frozen published
//       contract, which is the failure this gate exists to make impossible.
...
//
//   PRE-GA, A SUBTRACTIVE REGEN IS PERMITTED — the additive-only rule binds from
//   v1.0.0 GA, which has not shipped (measured: `git tag` lists v1.0.0-alpha.1 …
//   v1.0.0-rc.5 and no GA). So the "must be additive only" review instruction
//   above is the POST-GA rule; today's is the CHANGELOG preamble's, which says
//   pre-GA "still narrows and corrects the contract in place", recorded under a
//   `Breaking`/`Changed` heading. This passage previously framed one 2026-07-07
//   regen as the single authorised exception and not a precedent, which the
//   record does not support: measured from `git log` over this golden, narrowing
//   regens have landed repeatedly since the first pre-release shipped on
//   2026-07-08 — 3f8932f added thirteen `additionalProperties: false`
//   declarations, be12ebd the project-form `ports: false`/`healthCheck: false`
//   clause. Each still needs a deliberate decision and a CHANGELOG entry; none
//   of them needs to be the last one until v1.0.0 is tagged, after which none is
//   permitted at all.
using System;
using System.IO;
