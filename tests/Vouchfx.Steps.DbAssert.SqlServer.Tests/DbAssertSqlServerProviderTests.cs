// Tests for DbAssertSqlServerProvider — bind/validate/schema/registry.
//
// Mirrors Vouchfx.Steps.DbAssert.Postgres.Tests/DbAssertPostgresProviderTests.cs.
//
// Covers:
//   1. Bind: full YAML step (target/query/parameters/expect.rowCount + expect.row)
//      is deserialised into the correct DbAssertSqlServerModel.
//   2. Bind: non-mapping node → safe empty model (defensive).
//   3. Bind: rowCount-only expect → null Row.
//   4. Validate: valid model + matching sqlserver dependency → IsValid.
...
//  14. Emit: helper class is named 'DbAssertSqlServer_Helpers' (§13.3.1 prefix rule).
//  15. Emit: step id with hyphens is sanitised to underscores in the StatementBlock.
//  16. Emit: RequiredUsings contains the Microsoft.Data.SqlClient namespace.
//  17. Resources: yields a sqlserver ResourceRequirement whose Name equals model.Target.
//  18. CompileReferenceAssemblies: contains the Microsoft.Data.SqlClient assembly.
//  19. Registry: provider discoverable via StepKindRegistry with key "db-assert.sqlserver".
...
namespace Vouchfx.Steps.DbAssert.SqlServer.Tests;
...
/// <summary>
/// Non-docker unit tests for <see cref="DbAssertSqlServerProvider"/>
/// (bind, validate, emit, schema, registry discoverability).
/// </summary>
public sealed class DbAssertSqlServerProviderTests
{
    ...
    /// <summary>
    /// A fully valid model whose target is declared as type "sqlserver" passes
    /// validation.
    /// </summary>
    [Fact]
    public void Validate_ValidModel_WithMatchingSqlServerDependency_IsValid()
    {
    ...
    /// feat/case-sensitive-kinds): "SqlServer" does not match the canonical "sqlserver".
    /// </summary>
    [Fact]
    ...
    public void Validate_EmptyExpect_ReturnsExpectMustSpecifyRowCountOrRow()
    {
        ...
        Assert.False(result.IsValid);
        Assert.Contains(result.Errors, e =>
            e.Contains("db-assert.sqlserver", StringComparison.Ordinal) &&
    ...
    public void Validate_TargetNotInDeclaredDependencies_IsInvalid()
    {
        ...
        Assert.Contains(result.Errors, e =>
            ...
            e.Contains("sqlserver dependency", StringComparison.Ordinal));
    }

    // ── 11. Validate: target declared as wrong type ───────────────────────────

    /// <summary>
    /// When the target is declared but its type is not "sqlserver", the validator
    ...
    /// 'DbAssertSqlServer_Helpers' (§13.3.1 provider-id prefix rule).
    /// </summary>
    [Fact]
    public void Emit_HelperClass_IsNamedDbAssertSqlServer_Helpers()
    {
        var model = new DbAssertSqlServerModel(
            Target: "mydb",
            Query: "SELECT 1",
    ...
    /// <summary>
    /// RequiredUsings must include the Microsoft.Data.SqlClient namespace so the
    /// emitted helper can resolve SqlConnection without a fully-qualified name in
    ...
    public void Emit_RequiredUsings_ContainsSqlClientNamespace()
    {
        ...
        Assert.Contains("Microsoft.Data.SqlClient", fragment.RequiredUsings,
    ...
    /// <summary>
    /// Resources() yields a single sqlserver ResourceRequirement whose Name equals
    /// model.Target.
    /// </summary>
    [Fact]
    public void Resources_YieldsSqlServerRequirementWithCorrectName()
    {
        var model = new DbAssertSqlServerModel(
            Target: "orders-db",
            Query: "SELECT 1",
        ...
        Assert.Single(reqs);
        Assert.Equal("sqlserver", reqs[0].Family);
    ...
    /// <summary>
    /// CompileReferenceAssemblies must contain the Microsoft.Data.SqlClient assembly
    /// so the Roslyn compiler can resolve SqlConnection in the emitted helper.
    /// </summary>
    [Fact]
    ...
    public void Provider_IsDiscoverableViaStepKindRegistry()
    {
        var registry = StepKindRegistry.BuildAndFreeze(
            new[] { typeof(DbAssertSqlServerProvider).Assembly });

        var found = registry.TryGet("db-assert.sqlserver", out var registered);

        Assert.True(found, "Expected 'db-assert.sqlserver' to be registered.");
        Assert.NotNull(registered);
        Assert.Equal("db-assert", registered!.Kind.Family);
        Assert.Equal("sqlserver", registered.Kind.Provider);
        Assert.IsType<DbAssertSqlServerProvider>(registered.Instance);
    }
    ...
    public void Provider_SchemaFragment_ContainsRowCount()
    {
        ...
        registry.TryGet("db-assert.sqlserver", out var registered);
