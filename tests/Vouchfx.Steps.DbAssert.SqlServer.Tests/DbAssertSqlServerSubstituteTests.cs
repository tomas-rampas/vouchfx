// S04-B-03 / H1 — DbAssertSqlServerProvider substitution emit tests (non-docker).
//
// Verifies that DbAssertSqlServerProvider.Emit:
//   • passes the raw query template as a literal to ExecuteAsync (H1 blast-radius fix);
//   • the ResolveIdentifier call lives in RequiredHelpers (DbAssertSqlServer_Helpers),
...
/// <summary>
/// S04-B-03 / H1: emit-lint tests for <see cref="DbAssertSqlServerProvider"/>
/// substitution wrapping.
/// </summary>
public sealed class DbAssertSqlServerSubstituteTests
{
    ...
    /// <see cref="CsxFragment.RequiredHelpers"/> (inside <c>DbAssertSqlServer_Helpers</c>)
    ...
    public void Emit_QueryText_IsRawTemplateInStatementBlock_NotResolveIdentifierExpression()
    {
        ...
        //     (DbAssertSqlServer_Helpers), not the StatementBlock.
    ...
    /// once in <see cref="CsxFragment.RequiredHelpers"/> (<c>DbAssertSqlServer_Helpers</c>).
    /// </summary>
    [Fact]
    ...
    /// <see cref="DbAssertSqlServerProvider.Emit"/> must contain <c>using var</c>.
    /// </summary>
    [Fact]
    ...
    public async Task Emit_InjectionValue_WritesEnvironmentError_BeforeDbConnection()
    {
        ...
        var fragment = new DbAssertSqlServerProvider().Emit(model, ctx);
        ...
        var vars = new Dictionary<string, object?>(StringComparer.Ordinal)
        {
            ...
            [Vouchfx.Engine.Abstractions.VarKeys.Connection("orders-db")] = "Server=dummy;Database=dummy;User Id=u;Password=p",
            // Injection value — contains a semicolon, which is outside [A-Za-z0-9_.].
            ["tbl"] = "orders; DROP TABLE x",
        };
        var globals = new Vouchfx.Engine.Abstractions.ScriptGlobalVariables(vars);

        // Must NOT throw — the helper catches the InvalidOperationException internally.
        await Vouchfx.Engine.Compilation.RoslynScriptCompiler.RunIsolatedAsync(compiled, globals);

        var safeId = CsxFragment.SanitiseId(stepId);
        var outcomeKey = Vouchfx.Engine.Abstractions.VarKeys.Outcome(safeId);

        Assert.True(vars.ContainsKey(outcomeKey),
            $"Expected Vars to contain outcome key '{outcomeKey}'. " +
            $"Actual keys: [{string.Join(", ", vars.Keys)}]");

        var outcome = Assert.IsType<Vouchfx.Engine.Abstractions.StepOutcome>(vars[outcomeKey]);
        // The injection must yield EnvironmentError (not a propagated exception).
        Assert.Equal(Vouchfx.Engine.Abstractions.Verdict.EnvironmentError, outcome.Verdict);
