// §17 connection-string-aware credential-redaction tests for MqExpectRabbitmqProvider.
//
...
/// <c>MqExpectRabbitmq_Helpers.RedactAmqpUri(string amqpUri, string message)</c>.
/// </summary>
public sealed class MqExpectRabbitmqConnAwareRedactionTests
{
    ...
    private static MqExpectRabbitmqModel GetModel(
        ...
        new MqExpectRabbitmqModel(target, queue, new RabbitmqMatch(payloadContains, null, null));
        ...
        string message)
    {
        ...
        var fragment = provider.Emit(GetModel(), new StubCompileContext("redact-conn-aware"));

        var usings = string.Join("\n", fragment.RequiredUsings.Select(u => $"using {u};"));
        var helpers = string.Join("\n", fragment.RequiredHelpers);
        const string scriptBody =
            "Vars[\"__result__\"] = MqExpectRabbitmq_Helpers.RedactAmqpUri(" +
    ...
    public async Task Emit_CompileAndRun_CredentialedUnreachableConn_s3cr3tAbsentFromObservation()
    {
        ...
        var provider = new MqExpectRabbitmqProvider();
        var fragment = provider.Emit(GetModel(), new StubCompileContext("conn-aware-e2e"));
