...
public sealed class MqExpectRabbitmqEmitTests
{
    ...
    // (MqExpectRabbitmqProvider.cs, the generic catch and the RedactAmqpUri body it calls),
    ...
    // the encoding or of the provider: "t0ps3cr3t" contains no character EscapeDataString
    ...
    public async Task Emit_CompileAndRun_CredentialedConnFails_CredentialAbsentFromObservation()
    {
        var model = GetModel();
        using var dead = DeadLoopbackEndpoint.Reserve();
        var amqpUri = $"amqp://admin:t0ps3cr3t@{dead.Authority}/";
        var vars = new Dictionary<string, object?>(StringComparer.Ordinal)
        {
        ...
        Assert.True(
            !obs.Contains("t0ps3cr3t", StringComparison.Ordinal),
        ...
        Assert.True(
            !obs.Contains("admin:t0ps3cr3t", StringComparison.Ordinal),
    ...
    public async Task Emit_CompileAndRun_MissingSecretInPayloadContains_ReturnsEnvironmentError_ReferenceOnly()
    {
        ...
        try
        {
            ...
            // calls Secret_Helpers.ResolveTemplate for payloadContains / header values /
            // json values BEFORE it constructs the ConnectionFactory, so resolution throws
            // SecretResolutionException before any IConnection exists
            // (MqExpectRabbitmqProvider.cs, the ResolveTemplate lines preceding the
            // `new RabbitMQ.Client.ConnectionFactory`).  The endpoint is a held reservation
            // anyway, so this file names no port it merely assumes is dead (#527).
            using var dead = DeadLoopbackEndpoint.Reserve();
            var vars = new Dictionary<string, object?>(StringComparer.Ordinal)
            {
