...
public sealed class MqPublishRedisRedactionTests
{
    ...
    /// still redacted by the regex fallback.  The crafted message separates its two
    /// tokens with ',', the only delimiter StackExchange.Redis's parser knows; it used
    /// to use ';', which only worked because the value bound wrongly stopped there
    /// (#553) -- a ';' is now part of the password value, as the parser treats it.
    /// </summary>
    [Fact]
